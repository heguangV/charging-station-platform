...
- 演示种子与结构迁移同批次执行：站点/设备/费率以 `INSERT OR IGNORE` 幂等对齐，演示用户集合则先行精确冲突校验（撞用户名/撞手机号即报错整体回滚，见 7.1），重复执行不产生重复数据；演示管理员由 v4 提供，SRS `UC-D-02` 的完整演示数据集（5 站点/48 桩/90 天历史）由 v8 灌入，口径见 7.1。注意 v1 迁移体在每次打开数据库时都会以 `INSERT OR IGNORE` 原样重放，因此 v8 已删除的遗留行可能在后续打开时借空出的 id 重新出现；v8 标记存在时每次打开都会重放遗留站清理策略以保持重开幂等（见 7.1）。
...
v8 在 v7 之后以与既有迁移相同的守卫模式执行一次：`schema_version` 无 `version=8` 行时才在 `BEGIN IMMEDIATE` 事务内灌入种子并写入标记行（`8/full-demo-seed/ncs-v8-full-demo-seed`，标记行的 `applied_at` 即历史锚点），失败整体回滚；标记行已存在时只重放上一节所述的遗留站清理。种子运行前先做精确集合冲突校验：既有账号的用户名为 `sim_owner_001…300` 或手机号为 `13800001001…13800001300` 时，迁移以明确冲突错误中止并整体回滚，不做启发式清理（形似名与其它号段不受影响）。标记行与种子数据同事务提交，因此丢失标记却残留种子行的库属外部篡改状态，同样以冲突报错中止，恢复路径是从备份还原后处理冲突账号再升级。
