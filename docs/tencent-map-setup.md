...
- `TENCENT_MAP_SERVER_KEY`：仅供 Crow 服务端调用地理编码和路线规划 WebService。不得传给用户端、管理端、HTML 或日志；按腾讯控制台能力限制允许来源或出口 IP。

真实值写入仓库根目录 `.env` 或服务器进程环境。`.env` 和本地配置模板均不提交 Git。
...
程序不会输出 Key。Widgets 地图组件只读取 `TENCENT_MAP_JS_KEY` 和 `TENCENT_MAP_JS_ORIGIN`；`TENCENT_MAP_SERVER_KEY` 只由 Crow 服务端读取。
...
```bash
/path/to/Qt/6.2.0/gcc_64/bin/qt-cmake -S . -B build-qt62 -G Ninja
cmake --build build-qt62 --target codex-qt-demo
```

正常桌面运行：

```bash
build-qt62/codex-qt-demo
```
