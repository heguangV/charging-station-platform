...
{
    BioPointer input(
        BIO_new_mem_buf(pem.constData(), static_cast<int>(pem.size())),
        &BIO_free);
    if (!input) {
...
void checkPrivateKeyPermissions(const ServerConfig &config)
{
    ...
    constexpr auto unsafePermissions =
...
void checkTlsMaterial(const ServerConfig &config)
{
    ...
    const auto certificate = parseCertificate(
        readPemFile(config.tlsCertificatePath, "TLS certificate"));
    const auto privateKey = parsePrivateKey(
        readPemFile(config.tlsPrivateKeyPath, "TLS private key"));
...
asio::ssl::context createTlsContext(const ServerConfig &config)
{
    ...
    if (SSL_CTX_set_min_proto_version(context.native_handle(), TLS1_2_VERSION) != 1
        || SSL_CTX_set_cipher_list(
               context.native_handle(), "ECDHE+AESGCM:ECDHE+CHACHA20") != 1
