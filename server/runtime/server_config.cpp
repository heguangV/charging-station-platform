...
constexpr std::array<EnvironmentSetting, 21> environmentSettings{{
    ...
    {"NCS_BLOCKING_WORKER_THREADS", "NCS_BLOCKING_WORKER_THREADS",
     "--blocking-worker-threads"},
    {"NCS_BLOCKING_QUEUE_CAPACITY", "NCS_BLOCKING_QUEUE_CAPACITY",
    ...
    {"NCS_WEBSOCKET_MAX_PAYLOAD", "NCS_WEBSOCKET_MAX_PAYLOAD",
     "--websocket-max-payload"},
    {"NCS_WEBSOCKET_QUEUE_CAPACITY", "NCS_WEBSOCKET_QUEUE_CAPACITY",
    ...
    {"NCS_CORS_ALLOWED_ORIGINS", "NCS_CORS_ALLOWED_ORIGINS",
     "--cors-allowed-origins"},
    {"NCS_TENCENT_MAP_KEY", "TENCENT_MAP_SERVER_KEY", "--tencent-map-key"},
    {"NCS_DASHBOARD_SNAPSHOT", "NCS_DASHBOARD_SNAPSHOT",
     "--dashboard-snapshot"},
...
ncs::infrastructure::files::LogLevel
parseLogLevel(const std::string_view value, const std::string_view source) {
...
std::string normalizePath(const std::string_view value,
...
std::vector<std::string> parseCorsOrigins(const std::string_view value,
  ...
  while (start <= value.size()) {
    ...
    const bool validScheme = url.scheme() == QStringLiteral("https") ||
                             url.scheme() == QStringLiteral("http");
    if (item.empty() || !url.isValid() || !validScheme ||
    ...
    origins.emplace_back(normalized.constData(),
...
unsigned long parseUnsigned(const std::string_view value,
...
DeploymentEnvironment parseEnvironment(const std::string_view value,
                                       const std::string_view source) {
...
std::string parseListenAddress(const std::string_view value,
                               const std::string_view source) {
  asio::error_code error;
  const auto address = asio::ip::make_address(value, error);
  if (error || address.is_unspecified() || address.is_multicast()) {
    throw ConfigError("invalid or uncontrolled listen address for " +
                      std::string(source));
  }
  return address.to_string();
}

void applySetting(ServerConfig &config, const std::string_view option,
  ...
  if (option == "--environment") {
    ...
    config.workerThreads =
        static_cast<unsigned int>(parseUnsigned(value, 2, 256, source));
    ...
    config.chargeTimeScale = static_cast<std::uint32_t>(parseUnsigned(
        value, 1, std::numeric_limits<std::uint32_t>::max(), source));
...
std::optional<StartupAction>
requestedAction(const std::vector<std::string> &arguments) {
...
EnvironmentEntries parseEnvironmentFile(const QByteArray &content) {
  ...
  for (const QByteArray &rawLine : lines) {
    std::string_view line(rawLine.constData(),
                          static_cast<std::size_t>(rawLine.size()));
...
// Resolves the environment-file layer. The file named by NCS_ENV_FILE is
// mandatory when specified; the implicit <asset-directory>/.env is optional.
std::optional<EnvironmentEntries>
loadEnvironmentFile(const ServerConfig &config,
...
ServerConfig::ServerConfig() {
  ...
  assetDirectory = utf8Path(
      QFileInfo(baseDirectory.absolutePath()).absoluteFilePath());
  const QDir secretDirectory(
      baseDirectory.filePath(QStringLiteral("secrets")));
  logDirectory =
      utf8Path(QFileInfo(baseDirectory.filePath(QStringLiteral("logs")))
                   .absoluteFilePath());
  databasePath = utf8Path(QFileInfo(baseDirectory.filePath(QStringLiteral(
                                        "data/charge_platform.db")))
                              .absoluteFilePath());
  tlsCertificatePath = utf8Path(
      QFileInfo(secretDirectory.filePath(QStringLiteral("ncs-dev-cert.pem")))
          .absoluteFilePath());
  tlsPrivateKeyPath = utf8Path(
      QFileInfo(secretDirectory.filePath(QStringLiteral("ncs-dev-key.pem")))
          .absoluteFilePath());
  dashboardSnapshotPath = utf8Path(QFileInfo(
      baseDirectory.filePath(QStringLiteral(
          "apps/dashboard/public/data/dashboard.json"))).absoluteFilePath());
  mlWorkerScript = utf8Path(QFileInfo(
      baseDirectory.filePath(QStringLiteral("ml/worker.py"))).absoluteFilePath());
  mlModelPath = utf8Path(QFileInfo(
      baseDirectory.filePath(QStringLiteral("ml/models/load_rf.pkl"))).absoluteFilePath());
}
...
StartupOptions parseStartupOptions(const std::vector<std::string> &arguments,
  ...
  const auto environmentFile =
      loadEnvironmentFile(result.config, environmentLookup);
  bool timeScaleExplicit = false;
  if (environmentFile) {
    for (const auto &setting : environmentSettings) {
      const auto found =
          environmentFile->find(std::string(setting.envFileVariable));
      if (found == environmentFile->end() || found->second.empty())
        continue;
      applySetting(result.config, setting.option, found->second,
                   setting.envFileVariable);
  ...
  if (result.config.demoCredentialsEnabled()) {
    asio::error_code error;
    const auto address = asio::ip::make_address(result.config.listenAddress,
                                                error);
    if (error || !address.is_loopback()) {
      throw ConfigError(
          "development demo credentials require a loopback listen address");
    }
  }
...
Options:
  ...
  --tencent-map-key <key>         Server-side geocoding key; empty keeps the preset fallback
  --help, -h
  --version
...
Environment variables:
  ...
  NCS_TLS_CERTIFICATE, NCS_TLS_PRIVATE_KEY,
