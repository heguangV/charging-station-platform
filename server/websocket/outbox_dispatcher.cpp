...
OutboxDispatcher::OutboxDispatcher(ChargingRepository& repository,
                                   const std::shared_ptr<core::application::EventHub> hub)
    : repository_(repository), hub_(std::move(hub))
{
}

// 出站事件投递的一轮循环：pollOutbox 取批→按事件类型补齐关联数据（enrich）→EventHub 发布
// →标记 delivered；本进程已发布但未落库的行只补标记、绝不重发，数据缺失/类型未知等毒丸行
// 直接标记 dead 消耗掉，异常行仅计自身一次重试；有投递时按节流间隔补发 dashboard.refresh。
std::size_t OutboxDispatcher::dispatchOnce(const std::chrono::system_clock::time_point now)
{
    ...
    for (const auto& row : batch)
    {
        try
        {
            ...
            else if (row.eventType == "flow.updated")
            {
                ...
                data["statusText"] =
                    QString::fromStdString(core::application::flowStatusText(row.toStatus));
                ...
                if (flow->chargerCode)
                {
                    data["chargerCode"] = QString::fromStdString(*flow->chargerCode);
                }
                if (flow->status == static_cast<int>(core::application::FlowStatus::Queued))
                {
                    const auto queue = repository_.queue(flow->stationId, flow->chargerType);
                    for (std::size_t position = 0; position < queue.size(); ++position)
                    {
                        if (queue[position] == row.aggregateId)
                        {
                            data["queuePosition"] = static_cast<qint64>(position + 1);
                            break;
                        }
                    }
                }
                if (hub_->publish("flow.updated", compact(data),
                                  EventScope{flow->userId, std::nullopt, true, false},
                                  row.createdAt))
                {
                    deliveredIds.push_back(row.id);
            ...
            else if (row.eventType == "charger.statusChanged")
            {
                ...
                if (parsed.ec != std::errc{} || parsed.ptr != last || chargerId <= 0)
                {
                ...
                if (hub_->publish("charger.statusChanged", compact(data),
    ...
    if (!deliveredIds.empty() && now - lastRefreshAt_ >= kRefreshThrottle)
    {
        lastRefreshAt_ = now;
        hub_->publish("dashboard.refresh", "{}",
                      EventScope{std::nullopt, std::nullopt, false, true}, nowSeconds);
    }
