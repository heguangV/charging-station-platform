...
std::string localServerUrl(const ncs::server::runtime::ServerConfig &config) {
  const std::string host = config.listenAddress.find(':') == std::string::npos
                               ...
                               : "[" + config.listenAddress + "]";
  return "https://" + host + ":" + std::to_string(config.port);
}
...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    requestPolicy.configure(*logger,
                            std::unordered_set<std::string>(
                                startup.config.corsAllowedOrigins.begin(),
    ...
    auto hub = std::make_shared<ncs::core::application::EventHub>(
        ncs::core::application::EventHubOptions{
            ...
            [&sessions](const std::string_view token) {
              return sessions
                  .authenticate(token, std::chrono::system_clock::now())
                  .has_value();
            },
        });
    ncs::core::application::VerificationCodeService verificationCodes(
        startup.config.demoCredentialsEnabled());
    ncs::infrastructure::sqlite::SqliteRepository repository(
        startup.config.databasePath);
    ...
    ncs::core::application::UserIdentityService userIdentity(
        repository, sessions, verificationCodes);
    ncs::core::application::IdempotencyService idempotency(&repository);
    ncs::server::controller::HealthRoutes healthRoutes(apiRoutes, repository,
                                                       sessions);
    ...
    ncs::core::application::StationService stationService(
        repository, geocoder, adjustmentLookup);
    ...
    ncs::server::websocket::ChargeProgressPusher progressPusher(
        chargeFlowService, hub);
    ncs::core::application::AdminAuthService adminAuthService(repository,
                                                              sessions);
    ncs::core::application::AdminUserService adminUserService(
        repository, chargeFlowService, sessions);
    ncs::core::application::AdminStationService adminStationService(
        repository, repository, chargeFlowService, businessNumbers);
    ncs::core::application::DashboardService dashboardService(
        repository, repository, repository, repository);
    ncs::infrastructure::files::FileModelArtifactStore modelArtifacts(
        startup.config.mlModelPath);
    ncs::core::application::MlService mlService(
        repository, repository, repository, businessNumbers, &modelArtifacts);
    ncs::server::runtime::MlProcessManager mlProcessManager(
        ncs::server::runtime::MlProcessOptions{
            startup.config.pythonExecutable,
            startup.config.mlWorkerScript,
            localServerUrl(startup.config),
            startup.config.tlsCertificatePath,
            startup.config.mlModelPath},
        sessions, repository, repository, &modelArtifacts);
    ncs::core::application::AdminOpsService adminOpsService(
        repository, repository, chargeFlowService, businessNumbers,
    ...
    ncs::core::application::BoundedExecutor blockingExecutor(
        startup.config.blockingWorkerThreads,
        startup.config.blockingQueueCapacity);
    ...
    ncs::server::controller::WalletRoutes walletRoutes(
        apiRoutes, walletService, sessions, blockingExecutor, idempotency);
    ncs::server::controller::StationRoutes stationRoutes(
        apiRoutes, stationService, sessions, blockingExecutor);
    ncs::server::controller::FlowRoutes flowRoutes(
        apiRoutes, chargeFlowService, sessions, blockingExecutor, idempotency);
    ...
    ncs::server::controller::AdminRoutes adminRoutes(
        apiRoutes, adminAuthService, adminUserService, adminStationService,
        adminOpsService, sessions, blockingExecutor, idempotency);
    ncs::server::controller::DashboardRoutes dashboardRoutes(
        apiRoutes, adminAuthService, dashboardService, sessions,
        blockingExecutor, startup.config.dashboardSnapshotPath, hub);
    ncs::server::controller::MlRoutes mlRoutes(
        apiRoutes, mlService, sessions, blockingExecutor, idempotency);
    ...
    scheduler.add(std::chrono::seconds(5), [&hub](const auto done) {
      hub->tickHeartbeat(
    ...
    scheduler.add(std::chrono::seconds(1), [&](const auto done) {
      if (!blockingExecutor.submit([&, done] {
            try {
    ...
    scheduler.add(std::chrono::seconds(2), [&](const auto done) {
      if (!blockingExecutor.submit([&, done] {
            try {
              outboxDispatcher.dispatchOnce(std::chrono::system_clock::now());
            } catch (...) {
            }
            done();
          })) {
    ...
    scheduler.add(std::chrono::seconds(15), [&](const auto done) {
      if (!blockingExecutor.submit([&, done] {
            try {
              ...
              // A transient SQLite failure must not permanently stall the
              // maintenance pass; the next tick retries.
            }
            done();
          })) {
        done();
    ...
    scheduler.add(std::chrono::seconds(30), [&](const auto done) {
      if (!blockingExecutor.submit([&, done] {
            try {
    ...
    scheduler.add(std::chrono::seconds(60), [&](const auto done) {
      if (!blockingExecutor.submit([&, done] {
            try {
    ...
    app.tick(std::chrono::seconds(1), [&scheduler] {
      scheduler.tick(std::chrono::steady_clock::now());
    });
    app.signal_clear()
        .signal_add(SIGINT)
        .signal_add(SIGTERM)
        ...
        .timeout(30)
        .server_name("")
        .ssl(ncs::server::runtime::createTlsContext(startup.config))
        .run();
