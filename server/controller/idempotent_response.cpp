...
// 幂等阻塞派发入口：校验 Idempotency-Key（必须为 UUID）后按 begin→租约→完成落库三步执行。
// Replay 直接回放已存储响应；InProgress/键冲突/租约失效映射为 IdempotencyConflict，待处理
// 容量超限返回 TransactionFailed 并附 Retry-After:1；Proceed 时持租约执行业务并落库供重放。
void dispatchIdempotentBlocking(
    ...
    core::application::IdempotencyService& idempotency, const std::string_view scope,
    ...
    std::function<crow::response(core::application::IdempotencyLease&)> operation)
{
    ...
    if (!key)
    {
        response = errorResponse(core::domain::ErrorCode::InvalidArgument,
                                 "idempotency key is missing or invalid", "缺少有效的幂等键");
        response.end();
        return;
    }
    dispatchBlocking(
        ...
        [&idempotency, scope = std::string(scope), key = std::string(*key),
         requestBody = request.body, permanent, operation = std::move(operation)]() mutable
        {
            const auto now = std::chrono::system_clock::now();
            const auto check = idempotency.begin(scope, key, requestBody, now, permanent);
            ...
            {
                auto unavailable =
                    errorResponse(core::domain::ErrorCode::TransactionFailed,
            ...
            case core::application::IdempotencyDecision::InvalidKey:
                return errorResponse(core::domain::ErrorCode::InvalidArgument,
                                     "idempotency key is invalid", "幂等键不符合要求");
            case core::application::IdempotencyDecision::Proceed:
                break;
            }

            core::application::IdempotencyLease lease(idempotency, scope, key, *check.leaseToken);
