...
QJsonObject quoteJson(const core::application::FlowQuoteView& quote)
{
    return {
        ...
        {QStringLiteral("chargerId"), QJsonValue(static_cast<qint64>(quote.chargerId))},
        {QStringLiteral("chargerCode"), QString::fromStdString(quote.chargerCode)},
        {QStringLiteral("electricityPriceCentPerKwh"), quote.electricityPriceCentPerKwh},
        {QStringLiteral("baseServicePriceCentPerKwh"), quote.baseServicePriceCentPerKwh},
        {QStringLiteral("queueAdjustmentBp"), quote.queueAdjustmentBp},
        {QStringLiteral("mlAdjustmentBp"), quote.mlAdjustmentBp},
        {QStringLiteral("finalServicePriceCentPerKwh"), quote.finalServicePriceCentPerKwh},
        {QStringLiteral("totalPriceCentPerKwh"), quote.totalPriceCentPerKwh},
        {QStringLiteral("expiresAt"), QJsonValue(static_cast<qint64>(quote.expiresAt))},
    };
}
...
QJsonObject flowJson(const core::application::FlowView& view)
{
    QJsonObject data{
        ...
        {QStringLiteral("stationId"), QJsonValue(static_cast<qint64>(view.stationId))},
        ...
        {QStringLiteral("version"), QJsonValue(static_cast<qint64>(view.version))},
    };
    if (view.chargerId)
        data.insert(QStringLiteral("chargerId"), QJsonValue(static_cast<qint64>(*view.chargerId)));
    else
        data.insert(QStringLiteral("chargerId"), QJsonValue(QJsonValue::Null));
    if (view.chargerCode)
        data.insert(QStringLiteral("chargerCode"), QString::fromStdString(*view.chargerCode));
    else
    ...
    if (view.startedAt)
        data.insert(QStringLiteral("startedAt"), QJsonValue(static_cast<qint64>(*view.startedAt)));
    else
...
FlowRoutes::FlowRoutes(ApiRoutes& routes, core::application::ChargeFlowService& flows,
                       ...
                       core::application::IdempotencyService& idempotency)
{
    ...
    routes.route("/user/flows")
        .methods(crow::HTTPMethod::POST)(
            [&flows, &sessions, &executor, &idempotency](const crow::request& request,
                                                         crow::response& response)
            {
                ...
                const auto parsed =
                    parseJsonObject(request, {"stationId", "chargerType", "preferredChargerId"},
                                           ...
                                           9007199254740991LL))
                    {
                        response =
                            errorResponse(core::domain::ErrorCode::ValidationFailed,
                                          "flow request is invalid", "充电请求内容不符合要求");
                        response.end();
                        return;
                    }
                ...
                dispatchIdempotentBlocking(
                    ...
                    {
                        const auto result =
                            flows.createFlow(userIdValue, stationId, chargerType,
                                             preferredChargerId, std::chrono::system_clock::now());
                        if (!result.ok())
    ...
    routes.route("/user/flows/active")
        .methods(crow::HTTPMethod::GET)(
            [&flows, &sessions](const crow::request& request)
            {
                ...
                if (!userId)
                    return failure;
                const auto view = flows.activeFlow(*userId, std::chrono::system_clock::now());
    ...
    routes.route("/user/flows/<string>")
        .methods(crow::HTTPMethod::GET)(
            [&flows, &sessions](const crow::request& request, std::string flowNo)
            {
                crow::response failure;
                const auto userId = requireUserId(request, sessions, failure);
                if (!userId)
    ...
    // POST /user/flows/<flowNo>/quote-confirmations：确认报价并锁定充电桩；flowVersion 作为
    // 乐观锁，版本过期返回 VersionConflict，幂等作用域按流程号隔离。
        ...
        .methods(crow::HTTPMethod::POST)(
                ...
                const crow::request& request, crow::response& response, std::string flowNo)
            {
                    ...
                    !validIntegerField(*parsed.object, "flowVersion", 1, 9007199254740991LL))
                {
                    response =
                        errorResponse(core::domain::ErrorCode::ValidationFailed,
                                      "quote confirmation is invalid", "报价确认内容不符合要求");
                    response.end();
                    return;
                }
                const std::int64_t userIdValue = *userId;
                ...
                dispatchIdempotentBlocking(
                    request, response, executor, idempotency,
                    "u" + std::to_string(userIdValue) + ":flow-confirm:" + flowNo, false,
                        ...
                        return successResponse(QJsonObject{
                            ...
                            {QStringLiteral("statusText"),
                             QString::fromStdString(
                                 core::application::flowStatusText(result.value->status))},
                            ...
                            {QStringLiteral("reservedUntil"),
                             QJsonValue(static_cast<qint64>(result.value->reservedUntil))},
    ...
    routes.route("/user/flows/<string>/cancellations")
        .methods(crow::HTTPMethod::POST)(
                ...
                const crow::request& request, crow::response& response, std::string flowNo)
            {
                ...
                const auto parsed = parseJsonObject(request, {"reasonCode", "flowVersion"},
                                                    {"reasonCode", "flowVersion"});
                if (!parsed.object || !validStringField(*parsed.object, "reasonCode", 1, 32) ||
                    !validIntegerField(*parsed.object, "flowVersion", 1, 9007199254740991LL))
                {
                    response = errorResponse(core::domain::ErrorCode::ValidationFailed,
                                             "cancellation is invalid", "取消请求内容不符合要求");
                    response.end();
                    return;
                }
                const std::int64_t userIdValue = *userId;
    ...
    // POST /user/flows/<flowNo>/start：开始充电；flowVersion 必填作乐观锁，targetAmountCent
    // 与 balanceFloorCent 为可选的金额上限与余额下限（整数分），越界即 ValidationFailed。
    routes.route("/user/flows/<string>/start")
        .methods(crow::HTTPMethod::POST)(
                ...
                const crow::request& request, crow::response& response, std::string flowNo)
            {
                ...
                {
                    if (!validIntegerField(*parsed.object, "targetAmountCent", 1, 1000000000LL))
                    {
                        response =
                            errorResponse(core::domain::ErrorCode::ValidationFailed,
                                          "start request is invalid", "开始充电请求内容不符合要求");
                        response.end();
                        return;
                    }
                ...
                {
                    if (!validIntegerField(*parsed.object, "balanceFloorCent", 0, 1000000000LL))
                    {
                        response =
                            errorResponse(core::domain::ErrorCode::ValidationFailed,
                                          "start request is invalid", "开始充电请求内容不符合要求");
                ...
                dispatchIdempotentBlocking(
                    ...
                    {
                        const auto result =
                            flows.start(userIdValue, flowNo, flowVersion, targetAmountCent,
        ...
        .methods(crow::HTTPMethod::GET)(
            [&flows, &sessions](const crow::request& request, std::string flowNo)
            {
                ...
                return successResponse(QJsonObject{
                    ...
                    {QStringLiteral("orderNo"), QString::fromStdString(result.value->orderNo)},
                    {QStringLiteral("status"), result.value->status},
                    {QStringLiteral("statusText"),
    ...
    // POST /user/flows/<flowNo>/settlements：结算并返回收据；幂等标记为 permanent，结果永久
    // 可重放，防止重复结算产生二次扣款。
    routes.route("/user/flows/<string>/settlements")
        .methods(crow::HTTPMethod::POST)(
                ...
                const crow::request& request, crow::response& response, std::string flowNo)
            {
                ...
                const auto parsed = parseJsonObject(request, {"flowVersion", "reasonCode"},
                    ...
                    !validIntegerField(*parsed.object, "flowVersion", 1, 9007199254740991LL))
                {
                    response =
                        errorResponse(core::domain::ErrorCode::ValidationFailed,
                                      "settlement request is invalid", "结算请求内容不符合要求");
                    response.end();
                    return;
                }
                const std::int64_t userIdValue = *userId;
    ...
    // GET /user/orders：分页查询历史订单；sort 仅允许 createdAt/-createdAt，status 过滤限定
    // 60~90，fromAt 晚于 toAt 即 ValidationFailed，杜绝任意字段注入排序/过滤。
    routes.route("/user/orders")
        .methods(crow::HTTPMethod::GET)(
            [&flows, &sessions](const crow::request& request)
            {
                ...
                if (!userId)
                    return failure;
                const auto pagination = parsePagination(request, {"createdAt", "-createdAt"},
                                                        {"status", "fromAt", "toAt"});
                if (!pagination)
                {
                ...
                const auto result = flows.orders(
                    *userId,
                    status->has_value() ? std::optional<int>(static_cast<int>(**status))
                                        : std::nullopt,
                    fromAt->value_or(0), toAt->value_or(0), pagination->sort, pagination->page,
                    pagination->pageSize);
                if (!result.ok())
                {
                ...
                for (const auto& order : result.value->items)
                {
                    ...
                    if (order.startedAt)
                        ...
                        item.insert(QStringLiteral("startedAt"), QJsonValue(QJsonValue::Null));
                    if (order.endedAt)
                        item.insert(QStringLiteral("endedAt"),
                                    QJsonValue(static_cast<qint64>(*order.endedAt)));
                    else
                        item.insert(QStringLiteral("endedAt"), QJsonValue(QJsonValue::Null));
    ...
    routes.route("/user/orders/<string>")
        .methods(crow::HTTPMethod::GET)(
            [&flows, &sessions](const crow::request& request, std::string orderNo)
            {
                crow::response failure;
                const auto userId = requireUserId(request, sessions, failure);
                if (!userId)
