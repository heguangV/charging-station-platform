                                    ...
                                    crow::response& failure)
{
    ...
    if (!result.context)
        failure = serviceError(result.error);
...
UserIdentityRoutes::UserIdentityRoutes(ApiRoutes& routes,
                                       ...
                                       const bool developmentMode)
{
    routes.route("/user/auth/sms/code")
        .methods(crow::HTTPMethod::POST)(
            ...
            {
                const auto parsed =
                    parseJsonObject(request, {"phone", "purpose"}, {"phone", "purpose"});
                if (!parsed.object)
                    return serviceError(ErrorCode::InvalidArgument);
                const auto result = identity.issueCode(text(*parsed.object, "phone"),
                                                       text(*parsed.object, "purpose"),
                                                       std::chrono::system_clock::now());
                if (!result.value)
                    return serviceError(result.error);
                QJsonObject data{
                    {QStringLiteral("expiresAt"),
                     QJsonValue(static_cast<qint64>(unixSeconds(result.value->expiresAt)))},
                    {QStringLiteral("retryAfterSec"),
                     result.value->retryAfterSec > 0 ? result.value->retryAfterSec : 60},
                    {QStringLiteral("maxAttempts"), 5},
                };
                if (developmentMode && loopback(request.remote_ip_address) &&
                    result.value->developmentCode)
                {
                    data.insert(QStringLiteral("developmentCode"),
                                QString::fromStdString(*result.value->developmentCode));
                }
                if (result.error == ErrorCode::RateLimited)
                {
                    auto response = errorResponse(
                        ErrorCode::RateLimited, "verification code cooldown", "请稍后再获取验证码",
                        QJsonObject{
                            {QStringLiteral("retryAfterSec"), result.value->retryAfterSec}});
        ...
        .methods(crow::HTTPMethod::POST)(
            [&identity, &blockingExecutor](const crow::request& request, crow::response& response)
            {
                const auto parsed = parseJsonObject(
                    request, {"username", "phone", "password", "smsCode", "deviceId"},
                ...
                dispatchBlocking(
                    request, response, blockingExecutor,
    ...
    routes.route("/user/auth/login/password")
        .methods(crow::HTTPMethod::POST)(
            [&identity, &blockingExecutor](const crow::request& request, crow::response& response)
            {
                const auto parsed = parseJsonObject(request, {"loginName", "password", "deviceId"},
                ...
                dispatchBlocking(request, response, blockingExecutor,
                                  ...
                                  deviceId = std::move(deviceId)]() mutable
                                 {
                                     const auto result = identity.loginPassword(
                                         std::move(loginName), std::move(password),
    ...
    routes.route("/user/auth/login/sms")
        .methods(crow::HTTPMethod::POST)(
            [&identity](const crow::request& request)
            {
                const auto parsed = parseJsonObject(request, {"phone", "smsCode", "deviceId"},
        ...
        .methods(crow::HTTPMethod::GET)(
            [&sessions](const crow::request& request)
            {
                ...
                for (const auto& session :
                     sessions.activeSessions(auth->principalId, std::chrono::system_clock::now()))
                {
                    items.append(QJsonObject{
                        {QStringLiteral("sessionId"),
                         QJsonValue(static_cast<qint64>(session.sessionId))},
                        {QStringLiteral("deviceId"), QString::fromStdString(session.deviceId)},
                        {QStringLiteral("createdAt"),
                         QJsonValue(static_cast<qint64>(unixSeconds(session.createdAt)))},
                        {QStringLiteral("lastSeenAt"),
                         QJsonValue(static_cast<qint64>(unixSeconds(session.lastSeenAt)))},
                        {QStringLiteral("expiresAt"),
                         QJsonValue(static_cast<qint64>(unixSeconds(session.expiresAt)))},
    ...
    routes.route("/user/sessions/<string>")
        ...
        .methods(crow::HTTPMethod::PUT)(
            [&identity, &sessions](const crow::request& request)
            {
                ...
                if (!auth)
                    return failure;
                const auto parsed =
                    parseJsonObject(request, {"nickname", "version"}, {"nickname", "version"});
    ...
    routes.route("/user/me/avatar")
        .methods(crow::HTTPMethod::POST)(
            [&identity, &sessions, &blockingExecutor](const crow::request& request,
    ...
    routes.route("/user/me/avatar/content")
        .methods(crow::HTTPMethod::GET)(
            [&identity, &sessions](const crow::request& request)
            {
                ...
                if (!profile.ok() || !profile.value->avatar)
    ...
    routes.route("/user/me/credential")
        .methods(crow::HTTPMethod::PUT)(
            [&identity, &sessions, &blockingExecutor](const crow::request& request,
                                                      crow::response& response)
            {
    ...
    routes.route("/user/me")
        .methods(crow::HTTPMethod::DELETE)(
                                                      ...
                                                      crow::response& response)
            {
                ...
                const auto parsed =
                    parseJsonObject(request, {"confirm", "password", "smsCode"}, {"confirm"});
                ...
                dispatchBlocking(
                     ...
                     smsCode = std::move(smsCode)]() mutable
                    {
                        const auto result = identity.deleteAccount(
                            ...
                            std::chrono::system_clock::now());
                        return result == ErrorCode::Ok ? successResponse() : serviceError(result);
                    });
            });
}
