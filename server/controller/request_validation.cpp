...
std::optional<int> queryInteger(const crow::request& request, const char* name, const int fallback)
{
...
// JSON 请求体统一校验：依次检查体积上限、Content-Type、可解析性、字段白名单与必填字段，
// 任一不满足即返回携带诊断信息的失败结果，路由层据此直接映射 InvalidArgument。
...
{
    const QJsonValue value = object.value(QString::fromUtf8(field.data(), field.size()));
    if (!value.isString())
        return false;
...
// 解析分页与查询参数：page/pageSize 必须为 ≥1 的整数且 pageSize ≤100，sort 必须命中白名单，
// 其余查询键必须命中过滤白名单且不得重复，任何违规返回 std::nullopt。
std::optional<Pagination> parsePagination(const crow::request& request,
...
// 从分页过滤集中解析整数过滤值：外层 nullopt 表示参数非法，内层空 optional 表示未提供该
// 过滤（允许缺省），提供了则必须完整解析且落在 [minimum, maximum] 区间。
...
// 提取幂等键：仅接受 Idempotency-Key 头中的合法 UUID，其余形式一律视为缺失。
