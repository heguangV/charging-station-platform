...
{
    const auto result =
        middleware::authorize(request, sessions, {core::application::TokenKind::Administrator},
                              {core::application::Role::Operator, core::application::Role::Owner},
                              std::chrono::system_clock::now());
    if (!result.context)
    {
        failure = errorResponse(result.error, "authentication failed", "管理员凭据无效或已过期");
        return std::nullopt;
    }
...
QJsonArray rolesJson(const std::vector<core::application::Role>& roles)
{
    ...
    for (const auto role : roles)
        array.append(QString::fromStdString(core::application::roleName(role)));
    return array;
}
...
QJsonObject adminJson(const core::application::AdminAccount& admin)
{
    return {
        ...
        {QStringLiteral("version"), QJsonValue(static_cast<qint64>(admin.version))},
    };
}
...
QJsonObject chargerJson(const core::application::Charger& charger)
{
    return {
        ...
        {QStringLiteral("statusText"), QString::fromStdString(core::application::chargerStatusText(
...
QJsonObject deviceCommandJson(const core::application::DeviceCommand& command)
{
    ...
    if (command.completedAt)
        data.insert(QStringLiteral("completedAt"),
                    QJsonValue(static_cast<qint64>(*command.completedAt)));
    else
...
QJsonObject flowJson(const core::application::ChargingFlow& flow)
{
    QJsonObject data{
        ...
        {QStringLiteral("chargerType"),
         flow.chargerType == core::application::ChargerType::DcFast ? 1 : 0},
...
QJsonObject auditJson(const core::application::AuditEvent& event)
{
    return {
        {QStringLiteral("actorId"),
         QString::fromStdString("admin:" + std::to_string(event.actorAdminId))},
...
AdminRoutes::AdminRoutes(ApiRoutes& routes, core::application::AdminAuthService& auth,
                         ...
                         core::application::AdminAccountService& accounts)
{
        ...
        .methods(crow::HTTPMethod::POST)(
            [&auth, &executor](const crow::request& request, crow::response& response)
            {
                ...
                dispatchBlocking(
                    ...
                    {
                        const auto result =
                            auth.login(std::move(username), std::move(password),
                                       std::move(deviceId), std::chrono::system_clock::now());
                        if (!result.ok())
                        ...
                        return successResponse(QJsonObject{
                            ...
                            {QStringLiteral("sessionId"),
                             QJsonValue(
                                 static_cast<qint64>(result.value->session.context.sessionId))},
                            {QStringLiteral("admin"), adminJson(result.value->admin)},
                        });
                    });
            });

    routes.route("/admin/auth/reauth")
        .methods(crow::HTTPMethod::POST)(
            [&auth, &sessions, &executor](const crow::request& request, crow::response& response)
            {
                crow::response failure;
                ...
                {
                    response =
                        errorResponse(ErrorCode::InvalidArgument, "reauth request is invalid",
                                      "重新验证请求内容不符合要求");
                    response.end();
                    return;
                }
                ...
                dispatchBlocking(request, response, executor,
                                 [&auth, context = *context, password]()
                                 {
                                     const auto result = auth.reauthenticate(
                                         context, password, std::chrono::system_clock::now());
                                     if (!result.ok())
                                         return errorResponse(result.error, "", "");
                                     return successResponse(QJsonObject{
                                         {QStringLiteral("reauthExpiresAt"),
                                          QJsonValue(static_cast<qint64>(*result.value))}});
                                 });
            });

    routes.route("/admin/auth/logout")
        .methods(crow::HTTPMethod::POST)(
    ...
    routes.route("/admin/users")
        .methods(crow::HTTPMethod::GET)(
            [&users, &sessions, &executor](const crow::request& request, crow::response& response)
            {
                crow::response failure;
                ...
                {
                    response = errorResponse(ErrorCode::ValidationFailed, "user filter is invalid",
                                             "用户过滤参数不符合要求");
                    response.end();
                    return;
                }
                    ...
                    [&users, query]
                    {
                        ...
                        for (const auto& user : result.items)
                        {
                            items.append(QJsonObject{
                                ...
                                {QStringLiteral("phoneMasked"),
                                 QString::fromStdString(maskedPhone(user.phone))},
                                {QStringLiteral("nickname"), QString::fromStdString(user.nickname)},
                                {QStringLiteral("status"), user.status},
                                {QStringLiteral("balanceCent"),
                                 QJsonValue(static_cast<qint64>(user.balanceCent))},
                                {QStringLiteral("debtCent"),
                                 QJsonValue(static_cast<qint64>(user.debtCent))},
                                {QStringLiteral("registeredAt"),
                                 QJsonValue(static_cast<qint64>(user.registeredAt))},
                            });
                        }
                        return successResponse(QJsonObject{
                ...
                dispatchBlocking(
                    ...
                    [&users, userId]
                    {
                        ...
                        QJsonObject data{
                            ...
                            {QStringLiteral("status"), user.status},
                            {QStringLiteral("statusText"),
                             user.status == 1 ? QStringLiteral("正常") : QStringLiteral("冻结")},
                            {QStringLiteral("balanceCent"),
                             QJsonValue(static_cast<qint64>(user.balanceCent))},
                            {QStringLiteral("debtCent"),
                             QJsonValue(static_cast<qint64>(user.debtCent))},
                            ...
                            {QStringLiteral("version"),
                             QJsonValue(static_cast<qint64>(user.version))},
                            {QStringLiteral("activeSessionCount"),
                             QJsonValue(static_cast<qint64>(result.value->activeSessionCount))},
                            {QStringLiteral("hasActiveFlow"),
                             result.value->activeFlow.hasActiveFlow},
                        };
                        return successResponse(std::move(data));
                    });
            });

    routes.route("/admin/users/<int>/status")
        .methods(crow::HTTPMethod::PUT)(
                ...
                const crow::request& request, crow::response& response, std::int64_t userId)
            {
                ...
                if (!actor)
                {
                    response = errorResponse(ErrorCode::Unauthorized, "authentication failed",
                                             "管理员凭据无效");
                    response.end();
                    return;
                }
                ...
                {
                    response =
                        errorResponse(ErrorCode::ValidationFailed, "status update is invalid",
                                      "冻结或解冻请求内容不符合要求");
                    response.end();
                    return;
                }
                ...
                dispatchIdempotentBlocking(
                    ...
                    "a" + std::to_string(actorValue) + ":user-status:" + std::to_string(userId),
                    false,
                    [&users, actorValue, userId, status, reason,
                     version](core::application::IdempotencyLease&)
                    {
                        const auto result =
                            users.updateStatus(actorValue, userId, status, reason, version,
                                               std::chrono::system_clock::now());
                        if (!result.ok())
                            return errorResponse(result.error, "", "");
                        return successResponse(QJsonObject{
                            {QStringLiteral("id"),
                             QJsonValue(static_cast<qint64>(result.value->user.id))},
                            {QStringLiteral("status"), result.value->user.status},
                            {QStringLiteral("statusText"), result.value->user.status == 1
                                                               ? QStringLiteral("正常")
                                                               : QStringLiteral("冻结")},
                            {QStringLiteral("version"),
                             QJsonValue(static_cast<qint64>(result.value->user.version))},
                            {QStringLiteral("activeFlowPreserved"),
                             result.value->activeFlowPreserved}});
                    });
            });

    routes.route("/admin/users/<int>/orders")
        .methods(crow::HTTPMethod::GET)(
            [&users, &sessions, &executor](const crow::request& request, crow::response& response,
                                           std::int64_t userId)
            {
                ...
                const auto pagination = adminPagination(request, {"status", "fromAt", "toAt"},
                                                        {"createdAt", "-createdAt"});
                if (!pagination)
                {
                    response = errorResponse(ErrorCode::ValidationFailed,
                ...
                const bool statusAllowed =
                    status && (!*status || **status == 60 || **status == 70 || **status == 90);
                if (!actor || !statusAllowed || !fromAt || !toAt ||
                    (*fromAt && *toAt && **fromAt > **toAt))
                {
                    response = errorResponse(ErrorCode::ValidationFailed,
                                             "order filters are invalid", "订单过滤参数不符合要求");
                    response.end();
                    return;
                }
                const std::int64_t actorValue = *actor;
                const auto statusValue =
                    *status ? std::optional<int>(static_cast<int>(**status)) : std::nullopt;
                ...
                dispatchBlocking(
                    request, response, executor,
                    [&users, actorValue, userId, statusValue, fromValue, toValue, sort, page,
                     pageSize]
                    {
                        const auto result =
                            users.orders(actorValue, userId, statusValue, fromValue, toValue, sort,
                                         page, pageSize, std::chrono::system_clock::now());
                        if (!result.ok())
                            return errorResponse(result.error, "", "");
                        QJsonArray items;
    ...
    routes.route("/admin/stations")
        .methods(crow::HTTPMethod::GET)(
                                              ...
                                              crow::response& response)
            {
                ...
                if (!status ||
                    (adcode != pagination->filters.end() && !decimalDigits(adcode->second, 6)))
                {
                    response =
                        errorResponse(ErrorCode::ValidationFailed, "station filters are invalid",
                                      "站点过滤参数不符合要求");
                    response.end();
                    return;
                }
                const auto statusValue =
                    *status ? std::optional<int>(static_cast<int>(**status)) : std::nullopt;
                const auto adcodeValue =
                    pagination->filters.count("adcode")
                        ? std::optional<std::string>(pagination->filters.at("adcode"))
                        : std::nullopt;
                const std::string keyword = pagination->filters.count("keyword")
                                                ? pagination->filters.at("keyword")
                                                : std::string();
                const int page = pagination->page;
                const int pageSize = pagination->pageSize;
                dispatchBlocking(
                    request, response, executor,
                    [&stations, statusValue, adcodeValue, keyword, page, pageSize]
                    {
                        const auto result =
                            stations.stations(statusValue, adcodeValue, keyword, page, pageSize);
                        QJsonArray items;
                        for (const auto& station : result.items)
                        {
                            items.append(QJsonObject{
                                ...
                                {QStringLiteral("version"),
                                 QJsonValue(static_cast<qint64>(station.version))}});
                        }
                        return successResponse(
                            QJsonObject{{QStringLiteral("items"), items},
                                        {QStringLiteral("total"), result.total},
    ...
    routes.route("/admin/stations")
        .methods(crow::HTTPMethod::POST)(
            [&stations, &sessions, &executor, &idempotency](const crow::request& request,
                                                            crow::response& response)
            {
                crow::response failure;
                const auto context = requireAdmin(request, sessions, failure);
                ...
                const auto parsed =
                    parseJsonObject(request,
                                    {"code", "name", "address", "adcode", "latitudeE6",
                                     "longitudeE6", "businessHours", "initialCharger"},
                                    {"code", "name", "address", "adcode", "latitudeE6",
                                     "longitudeE6", "initialCharger"});
                if (!parsed.object || !validStringField(*parsed.object, "code", 2, 16) ||
                ...
                {
                    response =
                        errorResponse(ErrorCode::ValidationFailed, "station payload is invalid",
                                      "新增站点请求内容不符合要求");
                    response.end();
                    return;
                }
                const QJsonObject initialCharger =
                    parsed.object->value("initialCharger").toObject();
                if (!hasOnlyFields(initialCharger,
                                   {"count", "chargerType", "powerWatt", "connectorStandard"}) ||
                ...
                draft.latitudeE6 =
                    static_cast<std::int64_t>(parsed.object->value("latitudeE6").toDouble());
                draft.longitudeE6 =
                    static_cast<std::int64_t>(parsed.object->value("longitudeE6").toDouble());
                ...
                spec.powerWatt =
                    static_cast<std::int64_t>(initialCharger.value("powerWatt").toDouble());
                    ...
                    [&stations, actorValue, draft, spec](core::application::IdempotencyLease&)
                    {
                        ...
                        return successResponse(
                            QJsonObject{{QStringLiteral("id"),
                                        ...
                                        {QStringLiteral("code"),
                                         QString::fromStdString(result.value->code)},
    ...
    routes.route("/admin/stations/<int>")
        .methods(crow::HTTPMethod::PUT)(
                ...
                const crow::request& request, crow::response& response, std::int64_t stationId)
            {
                ...
                {
                    response =
                        errorResponse(ErrorCode::ValidationFailed, "station update is invalid",
                                      "修改站点请求内容不符合要求");
                    response.end();
                    return;
                }
                ...
                dispatchIdempotentBlocking(
                    ...
                    "a" + std::to_string(actorValue) +
                        ":station-update:" + std::to_string(stationId),
                    false,
                    [&stations, actorValue, stationId, patch,
                     version](core::application::IdempotencyLease&)
                    {
                        const auto result =
                            stations.updateStation(actorValue, stationId, patch, version,
                                                   std::chrono::system_clock::now());
                        if (!result.ok())
                            return errorResponse(result.error, "", "");
                        return successResponse(
    ...
    {
        return [&stations, &sessions, &executor, &idempotency, enabled](
                   const crow::request& request, crow::response& response, std::int64_t stationId)
        {
            crow::response failure;
            const auto context = requireAdmin(request, sessions, failure);
            ...
            const auto actor = adminId(*context);
            const auto parsed =
                parseJsonObject(request, {"reason", "version"}, {"reason", "version"});
            if (!parsed.object || !actor || !parsed.object->value("reason").isString() ||
                !validIntegerField(*parsed.object, "version", 1, 9007199254740991LL))
            {
                response =
                    errorResponse(ErrorCode::ValidationFailed, "station state change is invalid",
                                  "站点启停请求内容不符合要求");
                response.end();
                return;
            }
            ...
            dispatchIdempotentBlocking(
                ...
                false,
                [&stations, actorValue, stationId, enabled, reason,
                 version](core::application::IdempotencyLease&)
                {
                    const auto result =
                        stations.setStationEnabled(actorValue, stationId, enabled, reason, version,
                                                   std::chrono::system_clock::now());
                    if (!result.ok())
                        return errorResponse(result.error, "", "");
                    return successResponse(QJsonObject{
    ...
    routes.route("/admin/stations/<int>/disable")
        .methods(crow::HTTPMethod::POST)(stationEnabledRoute(false));
    routes.route("/admin/stations/<int>/enable")
        .methods(crow::HTTPMethod::POST)(stationEnabledRoute(true));

    routes.route("/admin/chargers")
        .methods(crow::HTTPMethod::GET)(
            [&stations, &sessions, &executor](const crow::request& request,
                                              crow::response& response)
            {
                ...
                const auto pagination =
                    adminPagination(request, {"stationId", "status", "chargerType", "keyword"});
                if (!pagination)
                {
                    response = errorResponse(ErrorCode::ValidationFailed,
                                             "unsupported paging or filter parameter",
                ...
                const auto stationId =
                    parseIntegerFilter(*pagination, "stationId", 1, 9007199254740991LL);
                ...
                {
                    response =
                        errorResponse(ErrorCode::ValidationFailed, "charger filters are invalid",
                                      "设备过滤参数不符合要求");
                    response.end();
                    return;
                }
                ...
                const auto typeValue = *chargerType
                                           ? std::optional<int>(static_cast<int>(**chargerType))
                                           : std::nullopt;
                const std::string keyword = pagination->filters.count("keyword")
                                                ? pagination->filters.at("keyword")
                                                : std::string();
                ...
                dispatchBlocking(
                    request, response, executor,
                    [&stations, stationValue, statusValue, typeValue, keyword, pageNumber, pageSize]
                    {
                        const auto page = stations.chargers(stationValue, statusValue, typeValue,
    ...
    routes.route("/admin/chargers/batch")
        .methods(crow::HTTPMethod::POST)(
            [&stations, &sessions, &executor, &idempotency](const crow::request& request,
                                                            crow::response& response)
            {
                ...
                const auto actor = adminId(*context);
                const auto parsed =
                    parseJsonObject(request, {"stationId", "chargers"}, {"stationId", "chargers"});
                ...
                {
                    response =
                        errorResponse(ErrorCode::ValidationFailed, "batch payload is invalid",
                                      "批量创建设备请求内容不符合要求");
                    response.end();
                    return;
                }
                ...
                for (const auto& value : parsed.object->value("chargers").toArray())
                {
                    ...
                    if (!hasOnlyFields(item,
                                       {"code", "chargerType", "powerWatt", "connectorStandard"}) ||
                    ...
                    charger.powerWatt =
                        static_cast<std::int64_t>(item.value("powerWatt").toDouble());
                    if (item.value("connectorStandard").isString())
                        charger.connectorStandard =
                            item.value("connectorStandard").toString().toStdString();
                ...
                dispatchIdempotentBlocking(
                    ...
                    [&stations, actorValue, stationId, drafts](core::application::IdempotencyLease&)
                    {
                        ...
                        return successResponse(QJsonObject{{QStringLiteral("created"), items}},
                                               201);
                    });
            });

    routes.route("/admin/chargers/<int>/status")
        .methods(crow::HTTPMethod::PUT)(
            [&stations, &sessions, &executor, &idempotency](
                const crow::request& request, crow::response& response, std::int64_t chargerId)
            {
                ...
                {
                    response = errorResponse(ErrorCode::ValidationFailed,
                                             "charger status update is invalid",
                                             "设备状态变更请求内容不符合要求");
                    response.end();
                    return;
                }
                ...
                dispatchIdempotentBlocking(request, response, executor, idempotency,
                                           "a" + std::to_string(actorValue) +
                                               ":charger-status:" + std::to_string(chargerId),
                                           false,
                                           [&stations, actorValue, chargerId, targetStatus, reason,
                                            version](core::application::IdempotencyLease&)
                                           {
                                               const auto result = stations.setChargerStatus(
                                                   actorValue, chargerId, targetStatus, reason,
                                                   version, std::chrono::system_clock::now());
                                               if (!result.ok())
                                                   return errorResponse(result.error, "", "");
    ...
    routes.route("/admin/chargers/<int>/restart-commands")
        .methods(crow::HTTPMethod::POST)(
            [&stations, &sessions, &executor, &idempotency](
                const crow::request& request, crow::response& response, std::int64_t chargerId)
            {
                ...
                {
                    response =
                        errorResponse(context.error, "reauthentication or credentials required",
                                      "需要重新验证管理员密码");
                    response.end();
                    return;
                }
                ...
                const auto parsed =
                    parseJsonObject(request, {"confirm", "reason"}, {"confirm", "reason"});
                if (!parsed.object || !actor || !parsed.object->value("confirm").isBool() ||
                    !parsed.object->value("confirm").toBool() ||
                    !parsed.object->value("reason").isString())
                {
                    response =
                        errorResponse(ErrorCode::ValidationFailed, "restart command is invalid",
                                      "远程重启请求必须二次确认并填写原因");
                    response.end();
                    return;
                }
                ...
                dispatchIdempotentBlocking(
                    request, response, executor, idempotency,
                    "a" + std::to_string(actorValue) + ":restart:" + std::to_string(chargerId),
                    false,
    ...
    routes.route("/admin/device-commands/<string>")
        .methods(crow::HTTPMethod::GET)(
            [&stations, &sessions, &executor](const crow::request& request,
                                              crow::response& response, std::string commandNo)
            {
                crow::response failure;
                if (!requireAdmin(request, sessions, failure))
                {
                ...
                dispatchBlocking(request, response, executor,
                                 ...
                                 {
                                     const auto result = stations.deviceCommand(
                                         commandNo, std::chrono::system_clock::now());
                                     if (!result.ok())
                                         return errorResponse(result.error, "", "");
    ...
    routes.route("/admin/tariffs")
        .methods(crow::HTTPMethod::GET)(
                                              ...
                                              crow::response& response)
            {
                ...
                const auto effectiveAt =
                    parseIntegerFilter(*pagination, "effectiveAt", 0, 4102444800LL);
                const auto adcode = pagination->filters.find("adcode");
                if (!effectiveAt ||
                    (adcode != pagination->filters.end() && !decimalDigits(adcode->second, 6)))
                {
                    response =
                        errorResponse(ErrorCode::ValidationFailed, "tariff filters are invalid",
                                      "价格过滤参数不符合要求");
                    response.end();
                    return;
                }
                ...
                dispatchBlocking(
                    ...
                    [&stations, adcodeValue, effectiveValue, page, pageSize]
                    {
                        ...
                        {
                            if (effectiveValue && (tariff.effectiveFrom > *effectiveValue ||
                        ...
                        for (std::size_t index = first;
                             index < filtered.size() &&
                             index < first + static_cast<std::size_t>(pageSize);
                             ++index)
    ...
    routes.route("/admin/tariffs")
        .methods(crow::HTTPMethod::POST)(
                                                            ...
                                                            crow::response& response)
            {
                ...
                const bool hasEffectiveTo = parsed.object &&
                                            parsed.object->contains("effectiveTo") &&
                                            !parsed.object->value("effectiveTo").isNull();
                if (!parsed.object || !actor || !validStringField(*parsed.object, "adcode", 6, 6) ||
                    ...
                    (hasEffectiveTo &&
                     !validIntegerField(*parsed.object, "effectiveTo", 1, 4102444800LL)) ||
                    !validStringField(*parsed.object, "reason", 2, 200))
                {
                    response =
                        errorResponse(ErrorCode::ValidationFailed, "tariff payload is invalid",
                                      "价格版本请求内容不符合要求");
                    response.end();
                    return;
                }
                ...
                dispatchIdempotentBlocking(
                    ...
                    [&stations, actorValue, draft, reason](core::application::IdempotencyLease&)
                    {
                        const auto result = stations.createTariff(actorValue, draft, reason,
                                                                  std::chrono::system_clock::now());
                        if (!result.ok())
                            return errorResponse(result.error, "", "");
    ...
    routes.route("/admin/price-adjustments")
        .methods(crow::HTTPMethod::POST)(
            [&stations, &sessions, &executor, &idempotency](const crow::request& request,
                                                            crow::response& response)
            {
                ...
                const auto parsed =
                    parseJsonObject(request,
                                    {"stationId", "chargerType", "source", "adjustmentBp",
                                     "effectiveFrom", "effectiveTo", "reason"},
                                    {"stationId", "chargerType", "source", "adjustmentBp",
                                     "effectiveFrom", "effectiveTo", "reason"});
                if (!parsed.object || !actor ||
                ...
                {
                    response =
                        errorResponse(ErrorCode::ValidationFailed, "adjustment payload is invalid",
                                      "服务费调整请求内容不符合要求");
                    response.end();
                    return;
                }
                ...
                if ((draft.source != "ML_APPROVED" && draft.source != "MANUAL") ||
                    draft.adjustmentBp % 500 != 0 || draft.effectiveTo <= draft.effectiveFrom)
                {
                    response =
                        errorResponse(ErrorCode::ValidationFailed, "adjustment payload is invalid",
                                      "服务费调整请求内容不符合要求");
    ...
    routes.route("/admin/flows")
        .methods(crow::HTTPMethod::GET)(
            [&ops, &sessions, &executor](const crow::request& request, crow::response& response)
            {
                crow::response failure;
                if (!requireAdmin(request, sessions, failure))
                {
                ...
                const auto pagination =
                    adminPagination(request, {"status", "stationId", "chargerId", "userId"});
                if (!pagination)
                {
                    response = errorResponse(ErrorCode::ValidationFailed,
                                             "unsupported paging or filter parameter",
                ...
                const auto stationId =
                    parseIntegerFilter(*pagination, "stationId", 1, 9007199254740991LL);
                const auto chargerId =
                    parseIntegerFilter(*pagination, "chargerId", 1, 9007199254740991LL);
                const auto userId =
                    parseIntegerFilter(*pagination, "userId", 1, 9007199254740991LL);
                const bool statusAllowed =
                    status && (!*status || **status == 10 || **status == 20 || **status == 30 ||
                               **status == 40 || **status == 50 || **status == 60 ||
                               **status == 70 || **status == 80 || **status == 90);
                if (!statusAllowed || !stationId || !chargerId || !userId)
                {
                    response = errorResponse(ErrorCode::ValidationFailed,
                                             "flow filters are invalid", "流程过滤参数不符合要求");
                    response.end();
                    return;
                }
    ...
    routes.route("/admin/flows/<string>/force-releases")
        .methods(crow::HTTPMethod::POST)(
            [&ops, &sessions, &executor, &idempotency](const crow::request& request,
                                                       crow::response& response, std::string flowNo)
            {
                crow::response failure;
                const auto context = middleware::authorize(
                    request, sessions, {core::application::TokenKind::Administrator},
                ...
                {
                    response =
                        errorResponse(ErrorCode::ValidationFailed, "force release is invalid",
                                      "强制释放请求必须二次确认并填写原因");
                    response.end();
                    return;
                }
                ...
                dispatchIdempotentBlocking(
                    ...
                    [&ops, actorValue, flowNo, reason, nextStatus,
                     flowVersion](core::application::IdempotencyLease&)
                    {
                        const auto result =
                            ops.forceRelease(actorValue, flowNo, reason, nextStatus, flowVersion,
                                             std::chrono::system_clock::now());
                        if (!result.ok())
                            return errorResponse(result.error, "", "");
                        return successResponse(
                            QJsonObject{{QStringLiteral("flowNo"),
    ...
    routes.route("/admin/stats/revenue")
        .methods(crow::HTTPMethod::GET)(
            [&ops, &sessions, &executor](const crow::request& request, crow::response& response)
            {
                crow::response failure;
                ...
                const auto parameters =
                    parseQueryParameters(request, {"fromAt", "toAt", "stationId", "bucket"});
                const auto fromAt =
                    parameters ? parseIntegerParameter(*parameters, "fromAt", 0, 4102444800LL)
                               : std::optional<std::optional<std::int64_t>>{};
                const auto toAt = parameters
                                      ? parseIntegerParameter(*parameters, "toAt", 0, 4102444800LL)
                                      : std::optional<std::optional<std::int64_t>>{};
                const auto stationId = parameters ? parseIntegerParameter(*parameters, "stationId",
                                                                          1, 9007199254740991LL)
                                                  : std::optional<std::optional<std::int64_t>>{};
                const std::string bucket =
                    parameters && parameters->count("bucket") ? parameters->at("bucket") : "day";
                if (!parameters || !fromAt || !toAt || !stationId ||
                    !core::application::AdminOpsService::validBucket(bucket) ||
                    (*fromAt && *toAt && **fromAt > **toAt))
                {
                    response =
                        errorResponse(ErrorCode::ValidationFailed,
                                      "statistics parameters are invalid", "统计参数不符合要求");
                    response.end();
                    return;
                }
                const std::int64_t fromValue = *fromAt ? **fromAt : 0;
                ...
                dispatchBlocking(
                    ...
                    {
                        const auto result =
                            ops.revenueStats(fromValue, toValue, stationValue, bucket);
                        if (!result.ok())
                            return errorResponse(result.error, "", "");
                        QJsonArray items;
                        ...
                        {
                            items.append(
                                QJsonObject{{QStringLiteral("bucketStart"),
                                             QJsonValue(static_cast<qint64>(entry.bucketStart))},
                                            {QStringLiteral("amountCent"),
                                             QJsonValue(static_cast<qint64>(entry.amountCent))},
                                            {QStringLiteral("energyMwh"),
                                             QJsonValue(static_cast<qint64>(entry.energyMwh))},
    ...
    routes.route("/admin/stats/charger-status")
        .methods(crow::HTTPMethod::GET)(
            [&ops, &sessions, &executor](const crow::request& request, crow::response& response)
            {
                crow::response failure;
    ...
    // UC-A-09 管理员账号管理. Write routes require OWNER plus a recent
    // reauthentication; the response of every route is the canonical
    // AdminAccount body (id, username, roles, status, mustChangePassword,
    // version).
    routes.route("/admin/accounts")
        .methods(crow::HTTPMethod::GET)(
            [&accounts, &sessions, &executor](const crow::request& request,
                                              crow::response& response)
            {
                crow::response failure;
                ...
                const auto pagination = adminPagination(request, {});
                if (!pagination)
                {
                    response = errorResponse(ErrorCode::ValidationFailed,
                                             "unsupported paging or filter parameter",
                ...
                core::application::AdminAccountQuery query;
                query.page = pagination->page;
                query.pageSize = pagination->pageSize;
                dispatchBlocking(request, response, executor,
                                 [&accounts, query]
                                 {
                                     const auto result = accounts.list(query);
                                     QJsonArray items;
                                     for (const auto& account : result.items)
                                         items.append(adminJson(account));
                                     return successResponse(QJsonObject{
                                         {QStringLiteral("items"), items},
                                         {QStringLiteral("total"), result.total},
                                         {QStringLiteral("page"), result.page},
    ...
    routes.route("/admin/accounts")
        .methods(crow::HTTPMethod::POST)(
            [&accounts, &sessions, &executor, &idempotency](const crow::request& request,
                                                            crow::response& response)
            {
                crow::response failure;
                const auto context = middleware::authorize(
                ...
                {
                    response =
                        errorResponse(context.error, "owner role or reauthentication required",
                                      "需要 OWNER 权限并重新验证");
                    response.end();
                    return;
                }
                ...
                const auto parsed = parseJsonObject(request, {"username", "password", "reason"},
                                                    {"username", "password", "reason"});
                if (!parsed.object)
                {
                    response =
                        errorResponse(ErrorCode::ValidationFailed, "account creation is invalid",
                                      "创建管理员账号请求内容不符合要求");
                    response.end();
                    return;
                }
                ...
                dispatchIdempotentBlocking(
                    request, response, executor, idempotency,
                    "a" + std::to_string(actorValue) + ":admin-account-create", false,
                    [&accounts, actorValue, username, password,
                     reason](core::application::IdempotencyLease&)
                    {
                        const auto result = accounts.create(actorValue, username, password, reason,
                                                            std::chrono::system_clock::now());
                        if (!result.ok())
                            return errorResponse(result.error, "", "");
                        return successResponse(adminJson(*result.value), 201);
                    });
            });

    routes.route("/admin/accounts/<int>/status")
        .methods(crow::HTTPMethod::PUT)(
            [&accounts, &sessions, &executor, &idempotency](
                const crow::request& request, crow::response& response, std::int64_t adminAccountId)
            {
                crow::response failure;
                const auto context = middleware::authorize(
                    request, sessions, {core::application::TokenKind::Administrator},
                ...
                dispatchIdempotentBlocking(request, response, executor, idempotency,
                                           "a" + std::to_string(actorValue) +
                                               ":admin-status:" + std::to_string(adminAccountId),
                                           false,
                                           [&accounts, actorValue, adminAccountId, status, reason,
                                            version](core::application::IdempotencyLease&)
                                           {
                                               const auto result = accounts.updateStatus(
                                                   actorValue, adminAccountId, status, reason,
                                                   version, std::chrono::system_clock::now());
                                               if (!result.ok())
                                                   return errorResponse(result.error, "", "");
                                               return successResponse(adminJson(*result.value));
                                           });
            });

    routes.route("/admin/me/password")
        .methods(crow::HTTPMethod::PUT)(
            [&accounts, &sessions, &executor](const crow::request& request,
                                              crow::response& response)
            {
                ...
                const auto parsed = parseJsonObject(request, {"currentPassword", "newPassword"},
                                                    {"currentPassword", "newPassword"});
                if (!parsed.object || !parsed.object->value("currentPassword").isString() ||
                    !parsed.object->value("newPassword").isString())
                {
                    response =
                        errorResponse(ErrorCode::ValidationFailed, "password change is invalid",
                                      "修改密码请求内容不符合要求");
                    response.end();
                    return;
                }
                const std::int64_t actorValue = *actor;
                const std::int64_t sessionId = context->sessionId;
                const std::string currentPassword =
                    parsed.object->value("currentPassword").toString().toStdString();
                const std::string newPassword =
                    parsed.object->value("newPassword").toString().toStdString();
                dispatchBlocking(request, response, executor,
                                 [&accounts, actorValue, sessionId, currentPassword, newPassword]
                                 {
                                     const auto result = accounts.changeOwnPassword(
                                         actorValue, sessionId, currentPassword, newPassword,
                                         std::chrono::system_clock::now());
                                     if (!result.ok())
                                         return errorResponse(result.error, "", "");
                                     return successResponse(adminJson(*result.value));
        ...
        .methods(crow::HTTPMethod::GET)(
            [&ops, &sessions, &executor](const crow::request& request, crow::response& response)
            {
                ...
                if (actorFilter != pagination->filters.end() && !actorFilter->second.empty())
                {
                    ...
                    actorValid =
                        !actor.empty() && actor.size() <= 16 &&
                        std::all_of(actor.begin(), actor.end(),
                                    [](unsigned char value) { return std::isdigit(value) != 0; });
                }
                if (!fromAt || !toAt || !actorValid || (*fromAt && *toAt && **fromAt > **toAt))
                {
                    response = errorResponse(ErrorCode::ValidationFailed,
                                             "audit filters are invalid", "审计过滤参数不符合要求");
                    response.end();
                    return;
                }
                ...
                dispatchIdempotentBlocking(
                    ...
                    [&ops, actorValue](core::application::IdempotencyLease&)
                    {
                        ...
                        if (!result.ok())
                            return errorResponse(result.error, "", "");
                        return successResponse(
                            QJsonObject{{QStringLiteral("backupNo"),
                                         QString::fromStdString(result.value->backupNo)},
                                        {QStringLiteral("status"),
                                         QString::fromStdString(result.value->status)},
                                        {QStringLiteral("checksum"),
                                         QString::fromStdString(result.value->checksum)}},
                            202);
                    });
            });
            ...
            [&ops, &sessions, &executor](const crow::request& request, crow::response& response)
            {
                ...
                dispatchBlocking(
                    ...
                    [&ops]
                    {
                        ...
                        {
                            items.append(QJsonObject{
                                {QStringLiteral("backupNo"),
                                 QString::fromStdString(record.backupNo)},
                                {QStringLiteral("status"), QString::fromStdString(record.status)},
                                {QStringLiteral("checksum"),
                                 QString::fromStdString(record.checksum)},
                                {QStringLiteral("sizeBytes"),
                                 QJsonValue(static_cast<qint64>(record.sizeBytes))},
                                {QStringLiteral("createdAt"),
                                 QJsonValue(static_cast<qint64>(record.createdAt))},
    ...
    routes.route("/admin/backups/<string>/verifications")
        .methods(crow::HTTPMethod::POST)(
            [&ops, &sessions, &executor, &idempotency](
                const crow::request& request, crow::response& response, std::string backupNo)
            {
                crow::response failure;
                ...
                {
                    response =
                        errorResponse(ErrorCode::ValidationFailed, "backup verification is invalid",
                                      "备份验证请求不符合要求");
                    response.end();
                    return;
                }
                ...
                dispatchIdempotentBlocking(
                    request, response, executor, idempotency,
                    "a" + std::to_string(actorValue) + ":backup-verify:" + backupNo, false,
                    [&ops, actorValue,
                     backupNo = std::move(backupNo)](core::application::IdempotencyLease&)
                    {
                        const auto result = ops.verifyBackup(actorValue, backupNo,
                                                             std::chrono::system_clock::now());
                        if (!result.ok())
                            return errorResponse(result.error, "", "");
                        return successResponse(
    ...
    routes.route("/admin/predictions")
        .methods(crow::HTTPMethod::GET)(
            [&ops, &sessions, &executor](const crow::request& request, crow::response& response)
            {
                crow::response failure;
                ...
                const auto parameters =
                    parseQueryParameters(request, {"stationId", "horizonHour", "fromAt"});
                const auto stationId = parameters ? parseIntegerParameter(*parameters, "stationId",
                                                                          1, 9007199254740991LL)
                                                  : std::optional<std::optional<std::int64_t>>{};
                ...
                const auto fromAt =
                    parameters ? parseIntegerParameter(*parameters, "fromAt", 0, 4102444800LL)
                               : std::optional<std::optional<std::int64_t>>{};
                const bool horizonAllowed =
                    horizon && (!*horizon || **horizon == 1 || **horizon == 6 || **horizon == 24);
                if (!parameters || !stationId || !horizonAllowed || !fromAt)
                {
                    response =
                        errorResponse(ErrorCode::ValidationFailed,
                                      "prediction parameters are invalid", "预测参数不符合要求");
                    response.end();
                    return;
                }
                std::optional<std::int64_t> stationValue;
                        ...
                        for (const auto& prediction : items)
                        {
                            array.append(QJsonObject{
                                {QStringLiteral("stationId"),
                                 QJsonValue(static_cast<qint64>(prediction.stationId))},
                                {QStringLiteral("horizonHour"), prediction.horizonHour},
                                {QStringLiteral("modelVersion"),
                                 QString::fromStdString(prediction.modelVersion)},
                                {QStringLiteral("generatedAt"),
                                 QJsonValue(static_cast<qint64>(prediction.generatedAt))},
                                {QStringLiteral("targetAt"),
                                 QJsonValue(static_cast<qint64>(prediction.targetAt))},
                                {QStringLiteral("predictedEnergyMwh"),
                                 QJsonValue(static_cast<qint64>(prediction.predictedEnergyMwh))},
                                {QStringLiteral("predictedIdleCount"),
                                 prediction.predictedIdleCount},
    ...
    routes.route("/admin/ml-tasks")
        .methods(crow::HTTPMethod::POST)(
                                                       ...
                                                       crow::response& response)
            {
                ...
                const auto actor = adminId(*context);
                const auto parsed =
                    parseJsonObject(request, {"taskType", "horizonHours"}, {"taskType"});
                if (!parsed.object || !actor ||
                ...
                {
                    response =
                        errorResponse(ErrorCode::ValidationFailed, "ml task payload is invalid",
                                      "ML 任务请求内容不符合要求");
                    response.end();
                    return;
                }
                ...
                dispatchIdempotentBlocking(
                    ...
                    [&ops, actorValue, taskType, horizonHours](core::application::IdempotencyLease&)
                    {
                        const auto result = ops.startMlTask(actorValue, taskType, horizonHours,
                                                            std::chrono::system_clock::now());
                        if (!result.ok())
                            return errorResponse(result.error, "", "");
                        return successResponse(
    ...
    routes.route("/admin/ml-tasks/<string>")
        .methods(crow::HTTPMethod::GET)(
            [&ops, &sessions, &executor](const crow::request& request, crow::response& response,
                                         std::string taskNo)
            {
                                 ...
                                 [&ops, taskNo = std::move(taskNo)]
                                 {
                                     ...
                                     return successResponse(QJsonObject{
                                         ...
                                         {QStringLiteral("status"),
                                          QString::fromStdString(result.value->status)},
                                         {QStringLiteral("modelVersion"),
                                          QString::fromStdString(result.value->modelVersion)}});
                                 });
            });
}
