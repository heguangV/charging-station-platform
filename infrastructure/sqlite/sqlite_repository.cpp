  ...
  explicit Connection(const std::string &path) {
    ...
    if (result != SQLITE_OK) {
      const std::string message =
          database_ ? sqlite3_errmsg(database_) : "open failed";
      if (database_)
...
public:
  ...
  void bind(const int index, const std::string_view value) {
    verify(sqlite3_bind_text(statement_, index, value.data(),
  ...
  void bindBlob(const int index, const std::vector<unsigned char> &value) {
    verify(sqlite3_bind_blob(statement_, index, value.data(),
                             static_cast<int>(value.size()), SQLITE_TRANSIENT));
  }
  ...
  double real(const int column) const { return sqlite3_column_double(statement_, column); }
...
auto useDatabase(const SqliteRepository *owner, const std::string &path,
                 Work &&work) {
...
void bindOptional(Statement &statement, const int index,
...
std::optional<std::int64_t> optionalInteger(const Statement &statement,
                                            const int column) {
  return statement.isNull(column)
             ? std::nullopt
             : std::optional<std::int64_t>(statement.integer(column));
}

std::optional<std::string> optionalText(const Statement &statement,
                                        const int column) {
  return statement.isNull(column)
             ? std::nullopt
...
UserAccount readUser(Statement &statement) {
  ...
  if (!statement.isNull(12)) {
    account.avatar =
        AvatarData{statement.blob(12), statement.text(13), statement.text(14)};
  }
  return account;
}
...
void bindManagedUserFilters(Statement &statement,
...
std::vector<Role> readAdminRoles(sqlite3 *database, const std::int64_t adminId) {
  Statement query(database,
                  "SELECT role FROM admin_role WHERE admin_id=? ORDER BY role");
...
std::vector<int> decodeHorizons(const std::string_view encoded) {
  ...
  while (start < encoded.size()) {
    ...
    const std::string_view item = encoded.substr(
        start, comma == std::string_view::npos ? encoded.size() - start
                                               : comma - start);
    int value = 0;
    const auto parsed =
...
bool restrictOwnerPermissions(const std::filesystem::path &path,
...
COMMIT;
  ...
  Statement migration(connection.get(),
...
COMMIT;
  ...
  Statement statusMigration(connection.get(),
...
COMMIT;
  ...
  Statement demoMigration(connection.get(),
...
COMMIT;
  ...
  Statement indexMigration(connection.get(),
...
COMMIT;
  ...
  Statement analyticsMigration(connection.get(),
...
COMMIT;
  ...
  Statement orderIndexMigration(connection.get(),
...
SqliteRepository::findById(const std::int64_t id) const {
  return useDatabase(
      this, databasePath_,
      [&](sqlite3 *database) -> std::optional<UserAccount> {
        ...
        query.bind(1, id);
        return query.row() ? std::optional<UserAccount>(readUser(query))
                           : std::nullopt;
      });
}

std::optional<UserAccount>
SqliteRepository::findByPhone(const std::string_view phone) const {
  return useDatabase(
      this, databasePath_,
      [&](sqlite3 *database) -> std::optional<UserAccount> {
        Statement query(database, (std::string(userSelect) +
                                   "WHERE u.phone=? AND u.deleted=0")
                                      .c_str());
        query.bind(1, phone);
        return query.row() ? std::optional<UserAccount>(readUser(query))
                           : std::nullopt;
...
std::optional<UserAccount>
SqliteRepository::findByLoginName(const std::string_view loginName) const {
  return useDatabase(
      this, databasePath_,
      [&](sqlite3 *database) -> std::optional<UserAccount> {
        Statement query(database,
...
AccountWriteResult
SqliteRepository::updateNickname(const std::int64_t id,
                                 const std::int64_t expectedVersion,
                                 std::string nickname, UserAccount &updated) {
  return useDatabase(this, databasePath_, [&](sqlite3 *database) {
...
AccountWriteResult SqliteRepository::updateStatus(const std::int64_t id,
                                                  const int status,
                                                  UserAccount &updated) {
  ...
  return useDatabase(this, databasePath_, [&](sqlite3 *database) {
    Statement update(database,
                     "UPDATE user_account SET status=?,version=version+1 WHERE "
                     "id=? AND deleted=0");
...
AccountWriteResult SqliteRepository::updateCredential(const std::int64_t id,
                                                      std::string username,
...
AccountWriteResult SqliteRepository::replacePasswordHash(
...
AccountWriteResult SqliteRepository::updateAvatar(const std::int64_t id,
                                                  AvatarData avatar,
                                                  UserAccount &updated) {
  if (!findById(id))
    return AccountWriteResult::NotFound;
...
AccountWriteResult SqliteRepository::anonymize(const std::int64_t id,
                                               UserAccount &updated) {
...
void SqliteRepository::applyWalletState(const std::int64_t userId,
                                        const std::int64_t balanceCent,
                                        const std::int64_t debtCent) {
  useDatabase(this, databasePath_, [&](sqlite3 *database) {
    Statement update(
        database,
        "UPDATE user_account SET balance_cent=?,debt_cent=? WHERE id=?");
...
void SqliteRepository::setActiveFlowFlag(const std::int64_t userId,
                                         const bool hasActiveFlow) {
  useDatabase(this, databasePath_, [&](sqlite3 *database) {
    Statement update(database,
...
void SqliteRepository::withReadTransaction(
...
WalletAccount SqliteRepository::wallet(const std::int64_t userId) {
  return useDatabase(this, databasePath_, [&](sqlite3 *database) {
    Statement query(database,
                    "SELECT user_id,balance_cent,debt_cent,version,updated_at "
...
std::vector<WalletTransaction> SqliteRepository::walletTransactions(
  ...
  return useDatabase(this, databasePath_, [&](sqlite3 *database) {
    ...
    while (query.row())
      values.push_back(WalletTransaction{
          ...
          static_cast<WalletTransactionType>(query.integer(3)),
          query.integer(4), query.integer(5), query.integer(6), query.text(7),
          query.integer(8)});
    return values;
  });
}
...
std::optional<Station> SqliteRepository::station(const std::int64_t stationId) {
  return useDatabase(
      this, databasePath_, [&](sqlite3 *database) -> std::optional<Station> {
        Statement query(database,
                        "SELECT "
                        "id,code,name,address,adcode,latitude_e6,longitude_e6,"
                        "business_hours,enabled,version FROM station WHERE id=?");
        query.bind(1, stationId);
        return query.row() ? std::optional<Station>(readStation(query))
                           : std::nullopt;
      });
}

std::vector<Charger>
SqliteRepository::chargers(const std::optional<std::int64_t> stationId,
...
std::optional<Charger> SqliteRepository::charger(const std::int64_t chargerId) {
  return useDatabase(
      this, databasePath_, [&](sqlite3 *database) -> std::optional<Charger> {
        Statement query(
            database,
            "SELECT "
        ...
        return query.row() ? std::optional<Charger>(readCharger(query))
                           : std::nullopt;
      });
}
...
SqliteRepository::effectiveTariff(const std::string &adcode,
...
void SqliteRepository::addFlow(const ChargingFlow &value) {
  useDatabase(this, databasePath_, [&](sqlite3 *database) {
    const std::string sql =
        std::string("INSERT INTO charging_flow(") + flowColumns +
        ") VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
...
void SqliteRepository::saveFlow(const ChargingFlow &value) {
  useDatabase(this, databasePath_, [&](sqlite3 *database) {
    Statement update(
        database, "UPDATE charging_flow SET "
...
std::optional<ChargingFlow> SqliteRepository::flow(const std::string &flowNo) {
  return useDatabase(this, databasePath_,
                     [&](sqlite3 *database) -> std::optional<ChargingFlow> {
                       ...
                       query.bind(1, flowNo);
                       return query.row()
                                  ? std::optional<ChargingFlow>(readFlow(query))
                                  : std::nullopt;
                     });
}

std::optional<ChargingFlow>
SqliteRepository::activeFlow(const std::int64_t userId) {
  return useDatabase(
      this, databasePath_,
      [&](sqlite3 *database) -> std::optional<ChargingFlow> {
        const std::string sql =
...
SqliteRepository::pollOutbox(const std::int64_t now, const int limit) {
...
void SqliteRepository::markOutboxAttempted(
...
void SqliteRepository::enqueue(const std::int64_t stationId,
...
void SqliteRepository::dequeue(const std::int64_t stationId,
                               const ChargerType type,
                               const std::string &flowNo) {
  useDatabase(this, databasePath_, [&](sqlite3 *database) {
...
SqliteRepository::order(const std::string &orderNo) {
  return useDatabase(
      this, databasePath_,
...
std::optional<ChargingOrder>
SqliteRepository::orderByFlow(const std::string &flowNo) {
  return useDatabase(
      this, databasePath_,
      [&](sqlite3 *database) -> std::optional<ChargingOrder> {
        const std::string sql = std::string("SELECT ") + orderColumns +
...
SqliteRepository::orders(const std::int64_t userId,
...
std::vector<UserAccount> SqliteRepository::listAccounts() {
  return useDatabase(this, databasePath_, [&](sqlite3 *database) {
    Statement query(database,
                    (std::string(userSelect) + "WHERE u.deleted=0 "
                                                "ORDER BY u.id")
                        .c_str());
...
bool SqliteRepository::addStation(Station &station) {
  return useDatabase(
      this, databasePath_, [&](sqlite3 *database) -> bool {
        Statement exists(database,
                         "SELECT 1 FROM station WHERE code=? LIMIT 1");
        exists.bind(1, station.code);
        if (exists.row())
          return false;
        Statement nextId(database,
                         "SELECT COALESCE(MAX(id),0)+1 FROM station");
        const std::int64_t assignedId =
            nextId.row() ? nextId.integer(0) : 1;
        Statement insert(database,
                         "INSERT INTO "
...
bool SqliteRepository::stationCodeExists(const std::string &code) {
  return useDatabase(this, databasePath_, [&](sqlite3 *database) -> bool {
...
bool SqliteRepository::addCharger(Charger &charger) {
  return useDatabase(
      this, databasePath_, [&](sqlite3 *database) -> bool {
        Statement exists(database,
                         "SELECT 1 FROM charger WHERE code=? LIMIT 1");
        exists.bind(1, charger.code);
        if (exists.row())
          return false;
        Statement nextId(database,
                         "SELECT COALESCE(MAX(id),0)+1 FROM charger");
        const std::int64_t assignedId =
            nextId.row() ? nextId.integer(0) : 1;
        Statement insert(database,
                         "INSERT INTO "
...
SqliteRepository::tariffVersions(std::optional<std::string> adcode) {
  return useDatabase(this, databasePath_, [&](sqlite3 *database) {
    Statement query(database,
                    "SELECT "
                    "adcode,electricity_cent_per_kwh,service_cent_per_kwh,"
    ...
    while (query.row())
      values.push_back(RegionTariff{
          query.text(0), static_cast<int>(query.integer(1)),
...
ReadinessStatus SqliteRepository::probeDatabase() {
  ...
  try {
    Connection connection(databasePath_);
    Statement version(connection.get(),
                      "SELECT version,checksum FROM schema_version ORDER BY "
                      "version DESC LIMIT 1");
    status.schemaVersion =
        version.row() && version.integer(0) == kLatestSchemaVersion &&
    ...
    Statement touch(connection.get(),
                    "UPDATE schema_version SET applied_at=applied_at "
                    "WHERE version=?");
...
std::int64_t
SqliteRepository::nextBusinessSequence(const std::string_view prefix,
...
SqliteRepository::loadIdempotencyRecord(const std::string_view scope,
...
void SqliteRepository::saveIdempotencyRecord(
    const PersistedIdempotencyRecord &record) {
  useDatabase(this, databasePath_, [&](sqlite3 *database) {
    Statement statement(
        database,
...
void SqliteRepository::cleanupIdempotencyRecords(const std::int64_t now) {
  useDatabase(this, databasePath_, [&](sqlite3 *database) {
    Statement statement(
        database,
        "DELETE FROM idempotency_record WHERE "
...
void SqliteRepository::ensureDevelopmentAdmin(const bool enabled) {
  if (!enabled) {
    useDatabase(this, databasePath_, [](sqlite3 *database) {
      Statement disable(database,
                        "UPDATE admin_account SET status=0 "
                        "WHERE is_demo=1 AND status<>0");
      disable.execute();
    });
    return;
  }
  const bool exists = useDatabase(this, databasePath_, [](sqlite3 *database) {
    Statement query(database,
  ...
  if (exists)
    return;
  const std::string passwordHash = PasswordHasher().hash(
      "123456", PasswordHasher::currentIterations, 6);
  withTransaction([&] {
    Statement insert(
        transactionContext.database,
...
SqliteRepository::findAdminByUsername(const std::string_view username) {
  return useDatabase(
      this, databasePath_,
...
std::optional<AdminAccount>
SqliteRepository::findAdminById(const std::int64_t id) {
  return useDatabase(this, databasePath_,
                     [&](sqlite3 *database) -> std::optional<AdminAccount> {
    Statement query(database,
                    "SELECT id,username,password_hash,status,"
...
std::optional<UserAccount>
SqliteRepository::findManagedUser(const std::int64_t id) {
  return findById(id);
}
...
AccountWriteResult SqliteRepository::updateManagedUserStatus(
    ...
    const std::string_view reason, const std::int64_t expectedVersion,
    const std::int64_t at, UserAccount &updated) {
  AccountWriteResult result = AccountWriteResult::NotFound;
  withTransaction([&] {
    Statement update(transactionContext.database,
    ...
    addAuditEvent(AuditEvent{actorAdminId,
                             status == 0 ? "USER_FROZEN" : "USER_UNFROZEN",
...
SqliteRepository::auditEvents(const AuditEventQuery &query) {
  return useDatabase(this, databasePath_, [&](sqlite3 *database) {
    Statement select(
        database,
        ...
        "LIMIT ? OFFSET ?");
    const std::string actor = query.actorId.rfind("admin:", 0) == 0
                                  ? query.actorId.substr(6)
                                  : query.actorId;
    select.bind(1, actor);
    select.bind(2, actor);
...
bool SqliteRepository::createStationWithChargers(
    Station &station, const InitialChargerSpec &spec) {
  ...
  for (int index = 1; index <= spec.count; ++index) {
    ...
    suffix << index;
    Charger charger;
    charger.code = station.code +
                   (spec.chargerType == ChargerType::DcFast ? "-DC-" : "-AC-") +
                   suffix.str();
...
std::vector<Station>
SqliteRepository::stations(const std::optional<int> status,
...
bool SqliteRepository::tariffOverlaps(const std::string &adcode,
...
std::int64_t
SqliteRepository::addPriceAdjustment(const PriceAdjustment &adjustment) {
  return useDatabase(this, databasePath_, [&](sqlite3 *database) {
    Statement insert(database,
                     "INSERT INTO price_adjustment(station_id,charger_type,"
...
std::optional<PriceAdjustment> SqliteRepository::effectivePriceAdjustment(
    const std::int64_t stationId, const int chargerType, const std::int64_t at) {
  return useDatabase(
      this, databasePath_, [&](sqlite3 *database) -> std::optional<PriceAdjustment> {
    Statement query(database,
                    "SELECT id,station_id,charger_type,source,adjustment_bp,"
    ...
    if (!query.row())
      return std::nullopt;
    return PriceAdjustment{query.integer(0), query.integer(1),
                           static_cast<int>(query.integer(2)), query.text(3),
                           static_cast<int>(query.integer(4)), query.integer(5),
                           query.integer(6), query.text(7)};
  });
}
...
void SqliteRepository::addDeviceCommand(const DeviceCommand &command) {
  useDatabase(this, databasePath_, [&](sqlite3 *database) {
    Statement insert(database,
                     "INSERT INTO device_command(command_no,charger_id,charger_code,"
...
SqliteRepository::deviceCommand(const std::string &commandNo) {
  return useDatabase(this, databasePath_,
                     [&](sqlite3 *database) -> std::optional<DeviceCommand> {
    Statement query(database,
                    "SELECT command_no,charger_id,charger_code,status,reason,"
...
void SqliteRepository::saveDeviceCommand(const DeviceCommand &command) {
  useDatabase(this, databasePath_, [&](sqlite3 *database) {
    Statement update(database,
                     "UPDATE device_command SET status=?,completed_at=?,"
...
std::vector<DeviceCommand>
SqliteRepository::dueDeviceCommands(const std::int64_t now) {
  return useDatabase(this, databasePath_, [&](sqlite3 *database) {
    Statement query(database,
                    "SELECT command_no,charger_id,charger_code,status,reason,"
                    "actor_id,created_at,execute_at,completed_at,error_summary "
...
std::optional<ChargingFlow>
SqliteRepository::activeFlowOnCharger(const std::int64_t chargerId) {
  return useDatabase(
      this, databasePath_, [&](sqlite3 *database) -> std::optional<ChargingFlow> {
        Statement query(
            database,
            (std::string("SELECT ") + flowColumns +
...
AdminFlowPage SqliteRepository::flows(const AdminFlowQuery &query) {
  return useDatabase(this, databasePath_, [&](sqlite3 *database) {
    ...
    Statement count(database,
                    (std::string("SELECT COUNT(*)") + predicate).c_str());
    ...
    Statement select(
        database,
        (std::string("SELECT ") + flowColumns + predicate +
...
std::vector<ChargingOrder> SqliteRepository::settledOrders(
    const std::int64_t fromAt, const std::int64_t toAt,
...
std::optional<MlTask>
SqliteRepository::runningMlTask(const std::string &taskType) {
  return useDatabase(this, databasePath_, [&](sqlite3 *database) -> std::optional<MlTask> {
    Statement query(database,
    ...
    return query.row() ? std::optional<MlTask>(readMlTask(query))
                       : std::nullopt;
  });
}

std::vector<MlTask> SqliteRepository::overdueMlTasks(
    const std::int64_t trainDeadline, const std::int64_t predictDeadline) {
  return useDatabase(this, databasePath_, [&](sqlite3 *database) {
    Statement query(database,
                    "SELECT task_no,task_type,status,model_version,horizon_hours,"
                    "created_at,finished_at,metrics_summary,error_summary FROM ml_task "
...
bool SqliteRepository::tryFinishMlTask(const MlTask &task,
                                       const bool allowTimedOut) {
  return useDatabase(this, databasePath_, [&](sqlite3 *database) {
    const char *sql = allowTimedOut
...
void SqliteRepository::addBackup(const BackupRecord &record) {
  useDatabase(this, databasePath_, [&](sqlite3 *database) {
    Statement insert(database,
                     "INSERT INTO backup_record(backup_no,status,checksum,size_bytes,"
...
std::optional<BackupRecord>
SqliteRepository::backup(const std::string &backupNo) {
  return useDatabase(this, databasePath_,
...
bool SqliteRepository::createBackupSnapshot(BackupRecord &record) {
  ...
  try {
    ...
    record.sizeBytes = static_cast<std::int64_t>(
        std::filesystem::file_size(destinationPath));
...
bool SqliteRepository::verifyBackupSnapshot(const BackupRecord &record) {
  if (record.storagePath.empty() || record.checksum.empty() ||
      record.sizeBytes <= 0)
    return false;
  ...
  const std::filesystem::path allowedDirectory =
      std::filesystem::weakly_canonical(
          ...
          (std::filesystem::path(databasePath_).filename().string() +
           ".backups"));
  ...
  const std::filesystem::path temporary =
      allowedDirectory /
      (record.backupNo + "-verify-" + secureRandomToken(12) + ".db");
  try {
    std::filesystem::copy_file(canonicalBackup, temporary,
...
void SqliteRepository::cleanupAdminRecords(const std::int64_t now) {
  ...
  useDatabase(this, databasePath_, [&](sqlite3 *database) {
    ...
    Statement deadOutbox(
        database,
        "DELETE FROM outbox_event WHERE delivery_status=2 AND created_at<?");
...
void SqliteRepository::refreshHourlyMetrics(const std::int64_t fromAt,
  ...
  for (std::int64_t chunkStart = firstHour; chunkStart <= lastHour;
       chunkStart += kChunk) {
    const std::int64_t chunkEnd =
        std::min(chunkStart + kChunk - 3600, lastHour);
    withTransaction([&] {
      useDatabase(this, databasePath_, [&](sqlite3 *database) {
        Statement upsert(
            database,
...
HourlyMetricPage SqliteRepository::hourlyMetrics(
    const std::int64_t fromAt, const std::int64_t toAt,
    const std::optional<std::int64_t> stationId, const std::string_view cursor,
    const int limit) {
  ...
  if (!cursor.empty()) {
    ...
    const auto first = std::from_chars(left.data(), left.data() + left.size(),
                                       cursorBucket);
    const auto second = std::from_chars(right.data(), right.data() + right.size(),
                                        cursorStation);
  ...
  return useDatabase(this, databasePath_, [&](sqlite3 *database) {
    ...
    while (query.row()) {
      page.items.push_back(HourlyMetric{
          ...
          static_cast<int>(query.integer(5)),
          static_cast<int>(query.integer(6)),
          static_cast<int>(query.integer(8)), query.integer(7)});
    }
    if (page.items.size() > static_cast<std::size_t>(limit)) {
      ...
      page.nextCursor = std::to_string(last.bucketAt) + ":" +
                        std::to_string(last.stationId);
    }
    return page;
  });
}
...
std::int64_t SqliteRepository::nextDashboardVersion() {
  return useDatabase(this, databasePath_, [&](sqlite3 *database) {
    Statement update(database,
                     "UPDATE dashboard_state SET data_version=data_version+1 "
...
void SqliteRepository::addModelVersion(const ModelVersion &version) {
  useDatabase(this, databasePath_, [&](sqlite3 *database) {
    Statement insert(
        ...
        "artifact_checksum,artifact_path,created_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
...
SqliteRepository::modelVersion(const std::string_view versionNo) {
  return useDatabase(this, databasePath_, [&](sqlite3 *database)
                         -> std::optional<ModelVersion> {
    Statement query(
        database,
        "SELECT version_no,task_no,algorithm,feature_schema_version,random_seed,"
    ...
    if (!query.row())
      return std::nullopt;
    return ModelVersion{
        query.text(0), query.text(1), query.text(2),
        static_cast<int>(query.integer(3)), query.integer(4), query.integer(5),
        query.integer(6), query.real(7), query.real(8), query.real(9),
        query.real(10), query.real(11), query.real(12),
        static_cast<int>(query.integer(13)), query.integer(14) != 0,
        query.text(15), query.text(16), query.integer(17)};
  });
}
...
std::optional<ModelVersion> SqliteRepository::latestQualifiedModel() {
  return useDatabase(this, databasePath_, [&](sqlite3 *database)
    ...
    Statement versionNo(
        database,
        "SELECT v.version_no FROM model_version v JOIN ml_task t "
...
void SqliteRepository::upsertPredictions(
  ...
  withTransaction([&] {
    useDatabase(this, databasePath_, [&](sqlite3 *database) {
      Statement upsert(
          ...
          "model_version_no,target_at) DO UPDATE SET generated_at=excluded.generated_at,"
          "horizon_hour=excluded.horizon_hour,predicted_energy_mwh=excluded.predicted_energy_mwh,"
          "predicted_free_count=excluded.predicted_free_count,is_peak=excluded.is_peak,stale=excluded.stale");
...
std::vector<LoadPrediction> SqliteRepository::predictions(
    const std::optional<std::int64_t> stationId,
    const std::optional<int> horizonHour, const std::int64_t fromAt) {
  return useDatabase(this, databasePath_, [&](sqlite3 *database) {
    Statement query(
        database,
        "SELECT station_id,horizon_hour,model_version_no,generated_at,target_at,"
    ...
    while (query.row()) {
      values.push_back(LoadPrediction{
          query.integer(0), static_cast<int>(query.integer(1)), query.text(2),
          query.integer(3), query.integer(4), query.integer(5),
          static_cast<int>(query.integer(6)), query.integer(7) != 0,
          query.integer(8) != 0});
    }
    return values;
  });
...
void SqliteRepository::cleanupAnalytics(const std::int64_t now) {
  ...
  useDatabase(this, databasePath_, [&](sqlite3 *database) {
    Statement predictions(database,
                          "DELETE FROM load_prediction WHERE target_at<?");
    predictions.bind(1, now - 90 * day);
    predictions.execute();
    Statement metrics(database,
                      "DELETE FROM station_hourly_metric WHERE bucket_at<?");
...
void SqliteRepository::pruneBackups() {
  ...
  for (const auto &record : records) {
    ...
    useDatabase(this, databasePath_, [&](sqlite3 *database) {
      Statement remove(database,
                       "DELETE FROM backup_record WHERE backup_no=?");
