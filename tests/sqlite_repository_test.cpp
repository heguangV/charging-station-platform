...
public:
  TemporaryDatabase()
      : path_(std::filesystem::temp_directory_path() /
              ("ncs-sqlite-" + std::to_string(::getpid()) + ".db")) {
    cleanup();
  }
...
private:
  void cleanup() const {
    ...
    std::filesystem::remove_all(
        path_.parent_path() / (path_.filename().string() + ".backups"), ignored);
  }
...
int main() {
  ...
  constexpr std::string_view idempotencyKey =
      "2cb640c6-6995-4be5-9161-f0e2c1210099";
  constexpr std::string_view rollbackIdempotencyKey =
      "2cb640c6-6995-4be5-9161-f0e2c1210098";

  {
    ...
    tests.check(repository.findAdminByUsername("admin").has_value() &&
                ...
                "dashboard/ML migration and development account exist");
    ...
    tests.check(wallet.recharge(userId, 10000, now).ok(),
                "wallet recharge is persisted");
    ...
    const auto lookup = [&repository](const std::int64_t stationId,
                                      const int chargerType,
                                      const std::int64_t at) {
      const auto active =
          repository.effectivePriceAdjustment(stationId, chargerType, at);
    ...
    repository.withTransaction(
        [&] { backup = adminOps.createBackup(1, now); });
    ...
    if (backup.ok()) {
      ...
      tests.check(verified.ok() &&
                      verified.value->verificationStatus == "SUCCEEDED",
                  "backup verifies from an isolated copy");
    }

    AdminStationService adminStations(repository, repository, flows, numbers);
    const auto restart =
        adminStations.createRestartCommand(1, 3, "持久化命令测试", now);
    tests.check(restart.ok() &&
                    repository.charger(3)->status == ChargerStatus::Restarting,
    ...
    const auto rollbackRetry = atomicIdempotency.begin(
        "atomic-recharge", rollbackIdempotencyKey, "{\"amountCent\":100}",
    ...
    tests.check(created.ok() && created.value->quote,
                "persistent flow receives a quote");
    flowNo = created.value->flowNo;
    const auto confirmed = flows.confirmQuote(
        userId, flowNo, created.value->quote->quoteNo, created.value->version,
        now + std::chrono::seconds(1));
    tests.check(confirmed.ok(),
                "persistent quote confirmation creates an order");
    orderNo = confirmed.value->orderNo;
    const auto started =
        flows.start(userId, flowNo, confirmed.value->version, std::nullopt,
    ...
    tests.check(activeCharger &&
                    repository.activeFlowOnCharger(*activeCharger)->flowNo ==
                        flowNo &&
    ...
    tests.check(repository.wallet(userId).balanceCent ==
                    originalWallet.balanceCent,
                "failed transaction rolls back all writes");

    IdempotencyService idempotency(&repository);
    const auto reservation = idempotency.begin(
        "u1:recharge", idempotencyKey, "{\"amountCent\":10000}", now, true);
    IdempotencyLease lease(idempotency, "u1:recharge",
                           std::string(idempotencyKey),
                           *reservation.leaseToken);
    tests.check(
        reservation.decision == IdempotencyDecision::Proceed &&
            lease.complete(
                StoredHttpResult{200, "application/json", "persisted"}, now),
...

  {
    ...
    IdempotencyService idempotency(&repository);
    const auto replay = idempotency.begin("u1:recharge", idempotencyKey,
                                          "{\"amountCent\":10000}", now, true);
    tests.check(replay.decision == IdempotencyDecision::Replay &&
                    replay.replay && replay.replay->body == "persisted",
                "idempotency result replays after repository restart");
    tests.check(repository.findById(userId).has_value(),
                "user survives repository restart");
    tests.check(repository.findAdminByUsername("admin").has_value() &&
    ...
    if (persistedBackup) {
      std::ofstream tamper(persistedBackup->storagePath,
                           std::ios::binary | std::ios::app);
      tamper.put('\0');
      tamper.close();
      AdminOpsService adminOps(repository, repository, flows, numbers);
      const auto rejected =
          adminOps.verifyBackup(1, backupNo, now + std::chrono::seconds(4));
      tests.check(rejected.error ==
                          ncs::core::domain::ErrorCode::TransactionFailed &&
                      repository.backup(backupNo)->verificationStatus == "FAILED",
                  "tampered backup cannot pass checksum and isolated verification");
    }
    const auto active =
        flows.activeFlow(userId, now + std::chrono::seconds(12));
    tests.check(active.hasActiveFlow && active.flow->status == 40,
                "charging flow survives repository restart");
    const auto progress =
        flows.progress(userId, flowNo, now + std::chrono::seconds(12));
    ...
    const auto settledEventsBefore = queryInteger(
        database.path(),
        "SELECT COUNT(*) FROM outbox_event WHERE event_type='order.settled'");
    executeSql(
        database.path(),
    ...
    const auto failedSettlement =
        flows.settle(userId, flowNo, active.flow->version, "USER_STOPPED",
                     now + std::chrono::seconds(62));
    const auto failedFlow =
        flows.activeFlow(userId, now + std::chrono::seconds(62));
    tests.check(failedSettlement.error ==
                        ncs::core::domain::ErrorCode::TransactionFailed &&
...

  {
    ...
    tests.check(timedOut && timedOut->status == "TIMED_OUT" &&
                    timedOut->finishedAt == 1788501000 &&
                    !repository.runningMlTask("PREDICT"),
    ...
    const std::vector<std::tuple<const char *, const char *, std::int64_t>>
        retentionFixtures{
    ...
    executeSql(database.path(),
               "DELETE FROM outbox_event WHERE reason_code='RETENTION'");

    repository.refreshHourlyMetrics(1788500000 - 7200, 1788500000);
    tests.check(
        queryInteger(database.path(),
                     "SELECT MAX(bucket_at) FROM station_hourly_metric") <
...

  {
    ...
    for (const auto &row : stale) staleIds.push_back(row.id);
    ...
    tests.check(
        rows.size() == 1 && rows.front().eventType == "charger.statusChanged"
            && rows.front().aggregateType == "charger"
            && rows.front().aggregateId == "1"
            && rows.front().fromStatus == 0 && rows.front().toStatus == 1
            && rows.front().reasonCode == "ALLOCATED"
            && rows.front().createdAt == t0
            && rows.front().deliveryStatus == 0,
    ...
    tests.check(
        due.size() == 2 && due.front().id < due.back().id,
    ...
    tests.check(repository.pollOutbox(t0 + 100, 2).size() == 2,
                "poll respects the limit");
    ...
    tests.check(
        remaining.size() == 1 && remaining.front().id == first[2].id,
    ...
    const std::string attemptedSql =
        "SELECT delivery_status FROM outbox_event WHERE id="
        + std::to_string(remaining.front().id);
    tests.check(
        none.empty() && queryInteger(database.path(), attemptedSql.c_str()) == 2,
    ...
    const std::string deadSql =
        "SELECT delivery_status FROM outbox_event WHERE id="
        + std::to_string(deadRows.front().id);
    tests.check(
        repository.pollOutbox(t0 + 100, 10).empty()
