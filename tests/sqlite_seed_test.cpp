...
// Opens the repository expecting the migration to fail; returns false when the
// open succeeded and stores the thrown message for the conflict assertion.
bool openThrows(const std::string& db, std::string& message)
{
    try
    {
        ncs::infrastructure::sqlite::SqliteRepository repository(db);
        return false;
    }
...
int main()
{
    ...
    {
        // Exact seed-identity collision: a pre-existing real user named
        // sim_owner_001 makes the v8 re-run fail loudly, keeps the database
        // at its pre-v8 state and never seeds partial rows.
        TemporaryDatabase database;
        const std::string& db = database.path();
        {
            SqliteRepository repository(db);
        ...
        unseed(db);
        executeSql(db, "INSERT INTO user_account(username,phone,nickname,status,registered_at,"
                       "balance_cent,debt_cent,has_active_flow,version,deleted) VALUES "
                       "('sim_owner_001','13700000001','真实车主',1,1788500000,5000,0,0,1,0)");
        executeSql(db, "INSERT INTO wallet_account(user_id,balance_cent,debt_cent,version,"
                       "updated_at) SELECT id,5000,0,1,registered_at FROM user_account WHERE "
                       "username='sim_owner_001'");
        std::string message;
        tests.check(openThrows(db, message) && message.find("seed conflict") != std::string::npos &&
                        message.find("sim_owner_001") != std::string::npos,
                    "a conflicting username aborts the v8 re-run with an explicit error");
        DatabaseReader reader(db);
        tests.check(reader.integer("SELECT COUNT(*) FROM user_account WHERE "
                                   "username='sim_owner_001'") == 1 &&
                        reader.integer("SELECT COUNT(*) FROM wallet_account WHERE user_id=(SELECT "
                                       "id FROM user_account WHERE username='sim_owner_001')") == 1 &&
                        reader.integer("SELECT MAX(version) FROM schema_version") == 7 &&
                        reader.integer("SELECT COUNT(*) FROM user_account WHERE username GLOB "
                                       "'sim_owner_[0-9][0-9][0-9]'") == 1 &&
                        reader.integer("SELECT COUNT(*) FROM station") == 3,
                    "the failed seed keeps the real account, its wallet and the pre-v8 "
                    "schema intact");
    }

    {
        // Exact seed-phone collision: a real account on 13800001001 aborts the
        // v8 re-run with an explicit error naming the phone and its owner.
        TemporaryDatabase database;
        const std::string& db = database.path();
        {
            SqliteRepository repository(db);
        ...
        executeSql(db, "INSERT INTO user_account(username,phone,nickname,status,registered_at,"
                       "balance_cent,debt_cent,has_active_flow,version,deleted) VALUES "
                       "('real_owner_001','13800001001','真实车主',1,1788500000,5000,0,0,1,0)");
        std::string message;
        tests.check(openThrows(db, message) && message.find("seed conflict") != std::string::npos &&
                        message.find("13800001001") != std::string::npos,
                    "a conflicting phone aborts the v8 re-run with an explicit error");
        DatabaseReader reader(db);
        tests.check(reader.integer("SELECT COUNT(*) FROM user_account WHERE "
                                   "phone='13800001001'") == 1 &&
                        reader.integer("SELECT MAX(version) FROM schema_version") == 7 &&
                        reader.integer("SELECT COUNT(*) FROM user_account WHERE username GLOB "
                                       "'sim_owner_[0-9][0-9][0-9]'") == 0,
                    "the failed seed keeps the real account and no simulated owners appear");
    }

    {
        // A look-alike username that only wildcard matching would catch seeds
        // cleanly and survives untouched: the conflict check is exact-set.
        TemporaryDatabase database;
        const std::string& db = database.path();
        {
            SqliteRepository repository(db);
        ...
        executeSql(db, "INSERT INTO user_account(username,phone,nickname,status,registered_at,"
                       "balance_cent,debt_cent,has_active_flow,version,deleted) VALUES "
                       "('simXowner_001','13700000002','形似车主',1,1788500000,5000,0,0,1,0)");
        {
            SqliteRepository repository(db);
        }
        DatabaseReader reader(db);
        tests.check(reader.rows("SELECT username||'|'||phone FROM user_account WHERE "
                                "username='simXowner_001'")
                            .front() == "simXowner_001|13700000002" &&
                        reader.integer("SELECT COUNT(*) FROM user_account WHERE username GLOB "
                                       "'sim_owner_[0-9][0-9][0-9]'") == 300 &&
                        reader.integer("SELECT COUNT(*) FROM user_account WHERE username GLOB "
                                       "'sim_owner_[0-9][0-9][0-9]' OR username='simXowner_001'") ==
                            301,
                    "a look-alike username survives the re-seed and does not trip the "
                    "exact-match conflict check");
    }

    {
        // Historical tariffs of the seed zones and a pre-existing BJN station
        // survive the re-seed: tariffs are additive and the station is reused
        // by code instead of being duplicated or overwritten.
        TemporaryDatabase database;
        const std::string& db = database.path();
        {
            SqliteRepository repository(db);
        ...
        unseed(db);
        executeSql(db, "INSERT INTO region_tariff(adcode,electricity_cent_per_kwh,"
                       "service_cent_per_kwh,effective_from,effective_to) VALUES "
                       "('110106',99,66,1234567890,1788500000)");
        executeSql(db, "INSERT INTO station(id,code,name,address,adcode,latitude_e6,"
                       "longitude_e6,business_hours,enabled) VALUES "
                       "(20,'BJN','历史北京南站','自定义历史地址','110106',39900000,116400000,"
                       "'00:00-24:00',1)");
        {
            SqliteRepository repository(db);
        }
        DatabaseReader reader(db);
        tests.check(reader.integer("SELECT COUNT(*) FROM station WHERE code='BJN'") == 1 &&
                        reader.rows("SELECT name FROM station WHERE code='BJN'").front() ==
                            "历史北京南站" &&
                        reader.integer("SELECT COUNT(*) FROM charger WHERE station_id=20") == 10 &&
                        reader.integer("SELECT COUNT(*) FROM charging_order WHERE station_id=20") >
                            0 &&
                        reader.integer("SELECT COUNT(*) FROM region_tariff WHERE adcode='110106' "
                                       "AND electricity_cent_per_kwh=99 AND "
                                       "service_cent_per_kwh=66 AND effective_from=1234567890") ==
                            1 &&
                        reader.integer("SELECT COUNT(*) FROM region_tariff WHERE adcode='110106' "
                                       "AND electricity_cent_per_kwh=80 AND effective_from=0") == 1,
                    "historical tariffs and a pre-existing BJN station survive the re-seed; "
                    "the station is reused by code and receives its seeded devices");
    }
