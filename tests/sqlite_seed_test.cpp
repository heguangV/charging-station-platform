...
int main()
{
...

    {
        ...
        tests.check(reader.integer("SELECT MAX(version) FROM schema_version") == 10 &&
                        reader.rows("SELECT name||'|'||checksum FROM schema_version WHERE "
                                    "version=8")
                                .front() == "full-demo-seed|ncs-v8-full-demo-seed" &&
                        reader.integer("SELECT COUNT(*) FROM schema_version") == 10 &&
...

    {
        ...
        unseed(db);
        {
            ...
            DatabaseReader reader(db);
            tests.check(reader.integer("SELECT COUNT(*) FROM schema_version") == 10 &&
