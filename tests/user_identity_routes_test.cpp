...
crow::response
call(ncs::server::ServerApp &app, const crow::HTTPMethod method,
     std::string url, std::string body = {}, std::string token = {},
...
std::string issueCode(ncs::server::ServerApp &app, const std::string_view phone,
  ...
  const auto response =
      call(app, crow::HTTPMethod::POST, "/api/v1/user/auth/sms/code",
...
std::string imageMultipart(
    const QImage &image, std::string &contentType,
  ...
  std::string body = "--ncs-test-boundary" +
...
int main() {
  ...
  server::controller::UserIdentityRoutes routes(api, identity, sessions,
                                                blockingExecutor, true);
  app.validate();
  ...
  tests.check(registerCode.size() == 6,
              "development code endpoint returns six-digit code");
  const auto registerResponse =
      call(app, crow::HTTPMethod::POST, "/api/v1/user/auth/register",
  ...
  const QJsonObject registerData =
      registerEnvelope.value(QStringLiteral("data")).toObject();
  const std::string token = registerData.value(QStringLiteral("accessToken"))
                                .toString()
                                .toStdString();
  ...
  const std::string secondToken =
      passwordData.value(QStringLiteral("accessToken"))
          .toString()
          .toStdString();
  const auto secondSession =
      passwordData.value(QStringLiteral("sessionId")).toInteger();
  tests.check(passwordLogin.code == 200 && !secondToken.empty() &&
                  passwordData.value(QStringLiteral("sessionId")).isDouble() &&
                  secondSession > 0,
              "password login succeeds with numeric session id");

  const auto resetUnknown =
      call(app, crow::HTTPMethod::POST, "/api/v1/user/auth/sms/code",
           R"({"phone":"19999999999","purpose":"RESET_PASSWORD"})");
  const auto resetKnown =
      call(app, crow::HTTPMethod::POST, "/api/v1/user/auth/sms/code",
  ...
  const auto me =
      call(app, crow::HTTPMethod::GET, "/api/v1/user/me", {}, token, {});
  const QJsonObject meData =
      envelope(me).value(QStringLiteral("data")).toObject();
  tests.check(me.code == 200 &&
                  meData.value(QStringLiteral("version")).toInt() == 1,
  ...
  const auto unknownField =
      call(app, crow::HTTPMethod::PUT, "/api/v1/user/me",
           R"({"nickname":"new","version":1,"admin":true})", token);
  tests.check(unknownField.code == 422,
              "profile update rejects unknown fields");
  ...
  const auto controlNickname =
      call(app, crow::HTTPMethod::PUT, "/api/v1/user/me",
           R"({"nickname":"bad\nname","version":2})", token);
  tests.check(controlNickname.code == 422,
              "nickname rejects control characters");
  const auto conflict = call(app, crow::HTTPMethod::PUT, "/api/v1/user/me",
                             R"({"nickname":"旧版本","version":1})", token);
  tests.check(conflict.code == 409 &&
                  envelope(conflict).value("code").toInt() == 22,
  ...
  tests.check(sessionItems.size() == 2,
              "session list returns both active terminals");
  const auto malformedSession =
      call(app, crow::HTTPMethod::DELETE, "/api/v1/user/sessions/not-a-number",
           {}, token, {});
  tests.check(malformedSession.code == 400,
              "session route rejects a malformed numeric id");
  const auto revoke = call(
      app, crow::HTTPMethod::DELETE,
  ...
  tests.check(revokedAccess.code == 401,
  ...
  const std::string invalidMultipart =
      ...
      "\r\nContent-Type: image/png\r\n\r\nnot-an-image\r\n--" +
      invalidBoundary + "--\r\n";
  const auto invalidAvatar = call(
      app, crow::HTTPMethod::POST, "/api/v1/user/me/avatar", invalidMultipart,
      token, "multipart/form-data; boundary=" + invalidBoundary);
  ...
  const auto avatarUpload =
      call(app, crow::HTTPMethod::POST, "/api/v1/user/me/avatar", multipart,
           token, multipartType);
  tests.check(avatarUpload.code == 200 && envelope(avatarUpload)
                                                  .value("data")
                                                  .toObject()
                                                  .value("version")
                                                  .toInt() == 3,
              "valid image is normalized and stored");
  auto avatar = call(app, crow::HTTPMethod::GET,
                     "/api/v1/user/me/avatar/content", {}, token, {});
  tests.check(
      avatar.code == 200 &&
          avatar.get_header_value("Content-Type") == "image/png" &&
  ...
  tests.check(conditionalResponse.code == 304 &&
                  conditionalResponse.body.empty(),
  ...
  const auto parameterizedAvatar =
      call(app, crow::HTTPMethod::POST, "/api/v1/user/me/avatar",
  ...
  const auto gradientUpload =
      call(app, crow::HTTPMethod::POST, "/api/v1/user/me/avatar",
           gradientMultipart, token, gradientType);
  tests.check(gradientUpload.code == 200,
              "oversized avatar is downscaled and stored");
  const auto gradientContent =
      call(app, crow::HTTPMethod::GET, "/api/v1/user/me/avatar/content", {},
           token, {});
  const QImage storedGradient =
      QImage::fromData(QByteArray::fromStdString(gradientContent.body));
  ...
  for (int y = 0; y < noise.height(); ++y) {
    for (int x = 0; x < noise.width(); ++x) {
      ...
      noise.setPixel(x, y,
                     qRgb((randomState >> 16) & 0xff, (randomState >> 8) & 0xff,
                          randomState & 0xff));
    }
  }
  std::string noiseType;
  const std::string noiseMultipart = imageMultipart(noise, noiseType);
  const auto noiseUpload =
      call(app, crow::HTTPMethod::POST, "/api/v1/user/me/avatar",
           noiseMultipart, token, noiseType);
  tests.check(noiseUpload.code == 422,
              "incompressible normalized avatar is rejected");

  const auto otherBeforeCredential = call(
      app, crow::HTTPMethod::POST, "/api/v1/user/auth/login/password",
  ...
  const auto credential = call(
      ...
      token);
  tests.check(credential.code == 200,
              "credential update verifies current password");
  const auto revokedByCredential = call(
      app, crow::HTTPMethod::GET, "/api/v1/user/me", {}, otherBeforeToken, {});
  tests.check(revokedByCredential.code == 401,
              "credential update revokes other sessions");
  const auto thirdLogin = call(
      app, crow::HTTPMethod::POST, "/api/v1/user/auth/login/password",
      R"({"loginName":"driver_new","password":"new-example-password","deviceId":"desktop-a3"})");
  const std::string thirdToken = envelope(thirdLogin)
                                     .value("data")
                                     .toObject()
                                     .value("accessToken")
                                     .toString()
                                     .toStdString();
  const auto logout = call(app, crow::HTTPMethod::POST,
                           "/api/v1/user/auth/logout", {}, thirdToken, {});
  const auto repeatedLogout =
      call(app, crow::HTTPMethod::POST, "/api/v1/user/auth/logout", {},
           thirdToken, {});
  tests.check(logout.code == 200 && repeatedLogout.code == 200,
              "logout is idempotent");

  const auto adminSession = sessions.issue(
      "9", "admin-console", core::application::TokenKind::Administrator,
      {core::application::Role::Operator}, std::chrono::system_clock::now(),
      std::chrono::hours(1));
  const auto adminLogout =
      call(app, crow::HTTPMethod::POST, "/api/v1/user/auth/logout", {},
           adminSession ? adminSession->accessToken : std::string(), {});
  tests.check(adminLogout.code == 200,
              "user logout responds uniformly to non-user tokens");
  tests.check(adminSession &&
                  sessions
                      .authenticate(adminSession->accessToken,
  ...
  const auto smsLogin =
      call(app, crow::HTTPMethod::POST, "/api/v1/user/auth/login/sms",
           "{\"phone\":\"13800138001\",\"smsCode\":\"" + smsCode +
               "\",\"deviceId\":\"desktop-sms\"}");
  tests.check(smsLogin.code == 200 &&
                  envelope(smsLogin)
                          .value("data")
                          .toObject()
                          .value("user")
  ...
  const auto deletion = call(
      app, crow::HTTPMethod::DELETE, "/api/v1/user/me",
      R"({"confirm":true,"password":"new-example-password","smsCode":null})",
      token);
  tests.check(deletion.code == 200, "confirmed account deletion succeeds");
  const auto afterDeletion =
      call(app, crow::HTTPMethod::GET, "/api/v1/user/me", {}, token, {});
  tests.check(afterDeletion.code == 401,
              "account deletion revokes all sessions");
  const auto deletedAvatar =
      call(app, crow::HTTPMethod::GET, "/api/v1/user/me/avatar/content", {},
           token, {});
  tests.check(deletedAvatar.code == 401,
              "deleted avatar is no longer accessible");
  ...
  tests.check(frozenLogin.code == 403 &&
                  envelope(frozenLogin).value("code").toInt() == 6,
  ...
  const auto blockedDeletion =
      call(app, crow::HTTPMethod::DELETE, "/api/v1/user/me",
           R"({"confirm":true,"password":"active-password","smsCode":null})",
           activeToken);
  tests.check(blockedDeletion.code == 409 &&
                  envelope(blockedDeletion).value("code").toInt() == 9,
  ...
  tests.check(accounts.anonymize(active.id, shouldRemainActive) ==
                  ...
                  accounts.findById(active.id) &&
                  !accounts.findById(active.id)->deleted,
