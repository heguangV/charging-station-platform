...
call(ncs::server::ServerApp &app, const crow::HTTPMethod method,
  ...
  crow::request request(method, url, path, crow::query_string(url), {}, body, 1,
                        1, true, false, false);
...
int main() {
  ...
  core::application::ChargeFlowService flowService(repository, accounts,
                                                   accounts, numbers, 60);

  server::ServerApp app;
  server::controller::ApiRoutes api(app);
  server::controller::UserIdentityRoutes identityRoutes(api, identity, sessions,
  ...
  if (registered.code != 201) {
    std::cerr << "DEBUG register code=" << registered.code
              << " body=" << registered.body << "\n";
    std::cerr << "DEBUG sms body len=" << registerCode.size() << "\n";
  }
  tests.check(registered.code == 201 && token.size() >= 43,
              "user registered for business tests");

  tests.check(call(app, crow::HTTPMethod::GET, "/api/v1/user/wallet").code ==
                  401,
  ...
  const auto recharge =
      call(app, crow::HTTPMethod::POST, "/api/v1/user/wallet/recharges",
           R"({"amountCent":5000})", token, kUuidA);
  tests.check(recharge.code == 200 && envelope(recharge)
                                              .value("data")
                                              .toObject()
                                              .value("balanceAfterCent")
                                              .toInt() == 5000,
  ...
  tests.check(conflict.code == 409 &&
                  envelope(conflict).value("code").toInt() == 14,
              "reused idempotency key with a different body conflicts");

  const auto overview =
      call(app, crow::HTTPMethod::GET, "/api/v1/user/wallet", {}, token);
  tests.check(overview.code == 200 && envelope(overview)
                                              .value("data")
                                              .toObject()
                                              .value("balanceCent")
                                              .toInt() == 5000,
  ...
  const auto transactions = call(app, crow::HTTPMethod::GET,
                                 "/api/v1/user/wallet/transactions", {}, token);
  tests.check(transactions.code == 200 &&
  ...
  tests.check(call(app, crow::HTTPMethod::GET,
                  ...
                  envelope(call(app, crow::HTTPMethod::GET,
                                "/api/v1/user/wallet/transactions?type=CHARGE",
                                {}, token))
                          .value("data")
                          .toObject()
                          .value("total")
  ...
  tests.check(call(app, crow::HTTPMethod::GET,
                   "/api/v1/user/wallet/transactions?type=NOPE", {}, token)
                      .code == 422,
              "unknown transaction type is rejected");
  tests.check(call(app, crow::HTTPMethod::GET,
                   "/api/v1/user/wallet/transactions?pageSize=101", {}, token)
                      .code == 422,
              "page size above the cap is rejected");
  tests.check(call(app, crow::HTTPMethod::GET,
                   "/api/v1/user/wallet/transactions?fromAt=not-a-number", {},
                   token)
                      .code == 422,
              "malformed wallet timestamp is rejected");

  const auto stations =
      call(app, crow::HTTPMethod::GET, "/api/v1/user/stations", {}, token);
  const QJsonObject stationsData = envelope(stations).value("data").toObject();
  tests.check(
      stations.code == 200 && stationsData.value("locationFallback").toBool() &&
          stationsData.value("items").toArray().size() >= 2,
      "nearby stations use the demo default location with a fallback flag");
  const QJsonObject firstStation =
      stationsData.value("items").toArray().first().toObject();
  ...
  tests.check(
      call(app, crow::HTTPMethod::GET, "/api/v1/user/stations/1", {}, token)
              .code == 200,
      "station detail is reachable");
  tests.check(
      call(app, crow::HTTPMethod::GET, "/api/v1/user/stations/999", {}, token)
              .code == 404,
      "unknown station is not found");
  const auto chargers =
      call(app, crow::HTTPMethod::GET,
           "/api/v1/user/stations/1/chargers?status=0", {}, token);
  tests.check(chargers.code == 200 && envelope(chargers)
                                              .value("data")
                                              .toObject()
                                              .value("items")
                                              .toArray()
  ...
  const auto quote =
      call(app, crow::HTTPMethod::GET,
           "/api/v1/user/stations/1/quote?chargerType=1", {}, token);
  tests.check(quote.code == 200 && envelope(quote)
                                           .value("data")
                                           .toObject()
                                           .value("totalPriceCentPerKwh")
                                           .toInt() == 135,
              "station quote returns the regional price breakdown");
  tests.check(call(app, crow::HTTPMethod::GET, "/api/v1/user/stations/1/quote",
                   {}, token)
                      .code == 422,
              "quote without a charger type is rejected");
  tests.check(call(app, crow::HTTPMethod::GET,
                   "/api/v1/user/stations?latitudeE6=39977680", {}, token)
                      .code == 422,
              "a partial coordinate pair is rejected");
  tests.check(call(app, crow::HTTPMethod::GET,
                   "/api/v1/user/stations/1/chargers?status=broken", {}, token)
                      .code == 422,
              "malformed charger status is rejected");

  const auto flowCreated =
      call(app, crow::HTTPMethod::POST, "/api/v1/user/flows",
           R"({"stationId":1,"chargerType":1,"preferredChargerId":null})",
           token, kUuidB);
  ...
  tests.check(flowCreated.code == 200 &&
                  flowData.value("status").toInt() == 20 &&
                  flowData.value("quote").toObject().contains(
                      QStringLiteral("quoteNo")),
              "flow request allocates a device and returns a quote");

  const auto active =
      call(app, crow::HTTPMethod::GET, "/api/v1/user/flows/active", {}, token);
  tests.check(active.code == 200 && envelope(active)
                                        .value("data")
                                        .toObject()
                                        .value("hasActiveFlow")
                                        .toBool(),
  ...
  tests.check(call(app, crow::HTTPMethod::POST,
                   ...
                   R"({"quoteNo":")" + quoteNo + R"(","flowVersion":99})",
                   token, kUuidC)
                      .code == 409,
  ...
  const auto confirmation =
      call(app, crow::HTTPMethod::POST,
           "/api/v1/user/flows/" + flowNo + "/quote-confirmations",
           R"({"quoteNo":")" + quoteNo + R"(","flowVersion":)" +
               std::to_string(flowVersion) + "}",
           token, kUuidE);
  const QJsonObject confirmationData =
      envelope(confirmation).value("data").toObject();
  const std::string orderNo =
      confirmationData.value("orderNo").toString().toStdString();
  tests.check(confirmation.code == 200 &&
                  confirmationData.value("status").toInt() == 30 &&
                  orderNo.size() >= 12,
              "quote confirmation creates the order and reserves the device");

  const auto start = call(
      app, crow::HTTPMethod::POST, "/api/v1/user/flows/" + flowNo + "/start",
      R"({"flowVersion":)" +
          std::to_string(confirmationData.value("version").toInt()) + "}",
      token, kUuidD);
  if (start.code != 200) {
  ...
  tests.check(
      start.code == 200 &&
          envelope(start).value("data").toObject().value("status").toInt() ==
              40,
      "start enters charging");

  const auto progress =
      call(app, crow::HTTPMethod::GET,
           "/api/v1/user/flows/" + flowNo + "/progress", {}, token);
  tests.check(progress.code == 200 &&
                  envelope(progress).value("data").toObject().contains(
                      QStringLiteral("simulatedSoc")) &&
                  envelope(progress)
                          .value("data")
                          .toObject()
                          .value("powerWatt")
  ...
  const auto settle =
      call(app, crow::HTTPMethod::POST,
           "/api/v1/user/flows/" + flowNo + "/settlements",
           R"({"flowVersion":99,"reasonCode":"USER_STOPPED"})", token, kUuidD);
  tests.check(settle.code == 409, "stale settlement version conflicts");

  const int startVersion =
      envelope(start).value("data").toObject().value("version").toInt();
  const auto retrySettle =
      call(app, crow::HTTPMethod::POST,
           "/api/v1/user/flows/" + flowNo + "/settlements",
           R"({"reasonCode":"USER_STOPPED","flowVersion":)" +
               std::to_string(startVersion) + "}",
           token, kUuidF);
  tests.check(retrySettle.code == 200 && envelope(retrySettle)
                                                 .value("data")
                                                 .toObject()
                                                 .value("status")
                                                 .toInt() == 60,
  ...
  const QJsonObject receipt =
      envelope(call(app, crow::HTTPMethod::GET,
                    "/api/v1/user/orders/" + orderNo, {}, token))
          .value("data")
          .toObject();
  ...
  const auto orders =
      call(app, crow::HTTPMethod::GET, "/api/v1/user/orders", {}, token);
  tests.check(
      orders.code == 200 &&
          envelope(orders).value("data").toObject().value("total").toInt() >= 1,
      "order list contains the settled order");
  tests.check(call(app, crow::HTTPMethod::GET, "/api/v1/user/orders?status=bad",
                   {}, token)
                      .code == 422,
              "malformed order status is rejected");
  const auto activeAfterSettle =
      call(app, crow::HTTPMethod::GET, "/api/v1/user/flows/active", {}, token);
  tests.check(envelope(activeAfterSettle)
                      .value("data")
                      .toObject()
                      .value("hasActiveFlow")
                      .toBool() == false,
