...
class TestRunner
{
  public:
    ...
    void expectConfigError(const std::function<void()>& operation, const std::string_view message)
    {
        try
        {
            operation();
...
int main(int argc, char* argv[])
{
    ...
    tests.check(defaults.config.environment == DeploymentEnvironment::Development,
    ...
    tests.check(!defaults.config.allowInsecureHttp, "insecure HTTP is disabled by default");
    tests.check(defaults.config.workerThreads == 2, "default worker count is two");
    tests.check(defaults.config.blockingWorkerThreads == 2 &&
    ...
    const auto productionDefaults =
        parseStartupOptions({"--environment", "production"}, lookupFor(emptyEnvironment));
    tests.check(productionDefaults.config.chargeTimeScale == 1,
                "non-development environments default to a real-time charge clock");
    const auto productionExplicit = parseStartupOptions(
        {"--environment", "production", "--charge-time-scale", "60"}, lookupFor(emptyEnvironment));
    tests.check(productionExplicit.config.chargeTimeScale == 60,
                "an explicit time scale still overrides the environment default");
    tests.check(defaults.config.websocketMaxConnections == 100 &&
    ...
    tests.check(fromEnvironment.config.environment == DeploymentEnvironment::Test,
                "environment values are applied");
    tests.check(!fromEnvironment.config.demoCredentialsEnabled(),
    ...
    tests.check(!fromEnvironment.config.allowInsecureHttp, "environment keeps HTTPS enabled");
    ...
    tests.check(envFileDirectory.isValid(), "temporary environment-file directory is available");
    ...
    envFile.write("# development defaults\n"
                  "NCS_PORT=8200\n"
    ...
    tests.check(withEnvFile.config.port == 8200, "environment-file values are applied");
    ...
    tests.check(withEnvFile.config.allowInsecureHttp,
    ...
    tests.check(!envBeatsFile.config.allowInsecureHttp,
                "process environment can restore secure transport");
    ...
    const auto cliBeatsAll =
        parseStartupOptions({"--port", "8443"}, lookupFor(fileThenEnvironment));
    ...
    tests.expectConfigError(
        ...
        {
            parseStartupOptions(
                {}, lookupFor({{"NCS_ENV_FILE",
                                utf8Path(QDir(envFileDirectory.path()).filePath("missing.env"))}}));
        },
    ...
    tests.check(overridden.config.environment == DeploymentEnvironment::Acceptance,
    ...
    tests.check(overridden.config.blockingWorkerThreads == 4 &&
                    overridden.config.blockingQueueCapacity == 128,
                "command line overrides blocking work limits");
    tests.check(overridden.config.corsAllowedOrigins ==
    ...
    tests.check(overridden.config.websocketMaxConnections == 200 &&
    ...
    tests.check(!overridden.config.allowInsecureHttp, "command line can explicitly retain HTTPS");
    tests.check(overridden.config.pythonExecutable == "python-cli" &&
                    overridden.config.mlWorkerScript.find("cli-worker.py") != std::string::npos,
    ...
    tests.expectConfigError(
        [&]
        {
            parseStartupOptions({"--cors-allowed-origins", "https://good.example/path"},
                                lookupFor(emptyEnvironment));
        },
    ...
    tests.expectConfigError(
        [&]
        {
            parseStartupOptions({"--environment", "test", "--allow-insecure-http", "true"},
                                lookupFor(emptyEnvironment));
        },
...
#endif
    tests.expectConfigError([&] { runStartupChecks(tlsConfig); },
                            "invalid PEM is rejected by startup checks");

    auto insecureConfig = defaults.config;
    insecureConfig.allowInsecureHttp = true;
    insecureConfig.port = 0;
    insecureConfig.tlsCertificatePath.clear();
    insecureConfig.tlsPrivateKeyPath.clear();
    try
    {
        runStartupChecks(insecureConfig);
        tests.check(true, "loopback development HTTP does not read TLS files");
    }
    catch (const ConfigError&)
    {
        tests.check(false, "loopback development HTTP does not read TLS files");
    }
    tests.expectConfigError(
        [&]
        {
            auto unsafeConfig = insecureConfig;
            unsafeConfig.environment = DeploymentEnvironment::Acceptance;
            runStartupChecks(unsafeConfig);
        },
        "startup checks defensively reject insecure HTTP outside development");
    tests.expectConfigError(
        [&]
        {
            auto unsafeConfig = insecureConfig;
            unsafeConfig.listenAddress = "192.0.2.10";
            runStartupChecks(unsafeConfig);
        },
        "startup checks defensively reject insecure HTTP off loopback");
    tests.check(privateKey.remove(), "temporary private key can be removed");
    tests.expectConfigError([&] { validateTlsFiles(tlsConfig); },
