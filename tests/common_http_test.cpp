...
int main()
{
    ...
    tests.check(core::domain::httpStatus(ErrorCode::InvalidArgument) == 400,
                "invalid argument maps 400");
    tests.check(core::domain::httpStatus(ErrorCode::ValidationFailed) == 422,
                "validation maps 422");
    tests.check(core::domain::httpStatus(ErrorCode::IdempotencyConflict) == 409,
                "idempotency maps 409");
    ...
    infrastructure::files::RequestLogScope responseScope("2cb640c6-6995-4be5-9161-f0e2c1201a53");
    const auto success =
        server::controller::successResponse(QJsonObject{{QStringLiteral("value"), 1}}, 201);
    ...
    const auto failure = server::controller::errorResponse(
        ErrorCode::InternalError, "SELECT token FROM /srv/private/database.sqlite", "retry");
    const auto failureBody = bodyObject(failure);
    tests.check(failure.code == 500 && failureBody.size() == 6, "failure uses six-field envelope");
    tests.check(!failure.body.empty() && failure.body.find("SELECT") == std::string::npos &&
    ...
    auto parsed =
        server::controller::parseJsonObject(jsonRequest, {"name", "version"}, {"name", "version"});
    tests.check(parsed.object.has_value(), "allowed JSON object is accepted");
    tests.check(parsed.object &&
                    server::controller::validStringField(*parsed.object, "name", 3, 32),
                "string field boundary is enforced");
    tests.check(parsed.object &&
                    server::controller::validIntegerField(*parsed.object, "version", 1, 100),
                "integer field boundary is enforced");
    jsonRequest.body = R"({"name":"valid","unknown":1})";
    tests.check(!server::controller::parseJsonObject(jsonRequest, {"name"}, {"name"}).object,
                "unknown JSON field is rejected");
    jsonRequest.body = R"({"name":)";
    tests.check(!server::controller::parseJsonObject(jsonRequest, {"name"}).object,
    ...
    const auto pagination =
        server::controller::parsePagination(pageRequest, {"createdAt", "-createdAt"}, {"status"});
    tests.check(pagination && pagination->page == 2 && pagination->pageSize == 100 &&
    ...
    tests.check(!server::controller::parsePagination(badPage, {"createdAt"}),
    ...
    tests.check(policyResponse.get_header_value("X-Request-ID") == policyContext.requestId,
                "request ID is returned in response");
    tests.check(policyResponse.get_header_value("Strict-Transport-Security") == "max-age=31536000",
                "TLS response includes HSTS");

    server::middleware::RequestPolicyMiddleware plainPolicy;
    plainPolicy.configure(logger, {}, false);
    server::middleware::RequestPolicyMiddleware::context plainContext;
    crow::request plainRequest = requestFor("/api/v1/system/health/live");
    crow::response plainResponse;
    plainPolicy.before_handle(plainRequest, plainResponse, plainContext);
    plainPolicy.after_handle(plainRequest, plainResponse, plainContext);
    tests.check(plainResponse.get_header_value("Strict-Transport-Security").empty(),
                "development HTTP responses do not advertise HSTS");
    ...
    tests.check(preflightResponse.code == 204 &&
    ...
    tests.check(server::middleware::RequestPolicyMiddleware::bodyLimitForPath(
    ...
    tests.check(server::middleware::RequestPolicyMiddleware::deadlineForPath(
    ...
    tests.check(timeoutResponse.code == 503 &&
    ...
    tests.check(limiter.allow("client", tick + std::chrono::seconds(1)).allowed,
                "rate token refills");
    ...
    tests.check(liveResponse.code == 200 &&
                    bodyObject(liveResponse).value(QStringLiteral("status")) ==
                        QStringLiteral("UP"),
    ...
    const auto admin =
        sessions.issue("admin-health", "ops-terminal", core::application::TokenKind::Administrator,
    ...
    if (admin)
        remoteReady.add_header("Authorization", "Bearer " + admin->accessToken);
