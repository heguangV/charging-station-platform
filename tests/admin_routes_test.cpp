...
int main()
{
    ...
    const auto registered = call(app, crow::HTTPMethod::POST, "/api/v1/user/auth/register",
                                 ...
                                 "\"password\":\"managed-password\",\"smsCode\":\"" +
                                     userCode + "\",\"deviceId\":\"managed-terminal\"}");
    const std::string userToken = envelope(registered)
                                      .value(QStringLiteral("data"))
                                      .toObject()
    ...
    tests.check(call(app, crow::HTTPMethod::GET, "/api/v1/admin/users").code == 401,
    ...
    const auto users =
        call(app, crow::HTTPMethod::GET, "/api/v1/admin/users?status=1", {}, adminToken);
    tests.check(users.code == 200 &&
                    envelope(users).value("data").toObject().value("total").toInt() == 1,
                "user list returns the managed account");
    tests.check(users.body.find("13800140001") == std::string::npos,
    ...
    const auto flowCreated =
        call(app, crow::HTTPMethod::POST, "/api/v1/user/flows",
             R"({"stationId":1,"chargerType":1,"preferredChargerId":null})", userToken, kKeyB);
    const QJsonObject flowData = envelope(flowCreated).value("data").toObject();
    const std::string flowNo = flowData.value(QStringLiteral("flowNo")).toString().toStdString();
    tests.check(flowCreated.code == 200 && flowData.value("status").toInt() == 20,
                "managed user holds a pending-quote flow");

    const auto disable = call(app, crow::HTTPMethod::POST, "/api/v1/admin/stations/1/disable",
                              R"({"reason":"维护停用","version":1})", adminToken, kKeyG);
    tests.check(disable.code == 409 && envelope(disable).value("code").toInt() == 15,
                "station with an active flow cannot be disabled");

    tests.check(call(app, crow::HTTPMethod::PUT, "/api/v1/admin/users/1/status",
    ...
    tests.check(freeze.code == 200 &&
                    envelope(freeze).value("data").toObject().value("activeFlowPreserved") == true,
                "freeze preserves the active charging flow");
    tests.check(call(app, crow::HTTPMethod::GET, "/api/v1/user/me", {}, userToken).code == 401,
    ...
    const int unfrozenVersion = envelope(freeze).value("data").toObject().value("version").toInt();
    tests.check(call(app, crow::HTTPMethod::PUT, "/api/v1/admin/users/1/status",
    ...
    tests.check(
        call(
            app, crow::HTTPMethod::POST, "/api/v1/admin/chargers/batch",
            R"({"stationId":1,"chargers":[{"code":"ZGC-DC-01","chargerType":1,"powerWatt":60000}]})",
            adminToken, kKeyA)
                .code == 409,
        "duplicate charger code is rejected in batch creation");

    tests.check(call(app, crow::HTTPMethod::PUT, "/api/v1/admin/chargers/1/status",
                     R"({"targetStatus":2,"reason":"人工巡检发现故障","version":1})", adminToken,
                     kKeyE)
                        .code == 409,
    ...
    tests.check(
        call(
            app, crow::HTTPMethod::POST, "/api/v1/admin/tariffs",
    ...
    tests.check(
        call(
            app, crow::HTTPMethod::POST, "/api/v1/admin/price-adjustments",
    ...
    tests.check(
        call(app, crow::HTTPMethod::POST, "/api/v1/admin/flows/" + flowNo + "/force-releases",
    ...
    const auto smsLogin =
        call(app, crow::HTTPMethod::POST, "/api/v1/user/auth/login/sms",
             "{\"phone\":\"13800140001\",\"smsCode\":\"" + smsCode(app, "13800140001", "LOGIN") +
                 "\",\"deviceId\":\"managed-terminal2\"}");
    const std::string newToken = envelope(smsLogin)
                                     .value("data")
                                     .toObject()
                                     .value(QStringLiteral("accessToken"))
                                     .toString()
    ...
    const auto newFlow =
        call(app, crow::HTTPMethod::POST, "/api/v1/user/flows",
             R"({"stationId":1,"chargerType":1,"preferredChargerId":null})", newToken, kKeyF);
    ...
    const auto confirmed = call(app, crow::HTTPMethod::POST,
                                "/api/v1/user/flows/" + newFlowNo + "/quote-confirmations",
                                R"({"quoteNo":")" +
                                    newFlowData.value("quote")
                                        .toObject()
    ...
    const auto started =
        call(app, crow::HTTPMethod::POST, "/api/v1/user/flows/" + newFlowNo + "/start",
             R"({"flowVersion":2})", newToken, kKeyA);
    tests.check(started.code == 200, "managed user started charging");

    const auto restart =
        call(app, crow::HTTPMethod::POST, "/api/v1/admin/chargers/2/restart-commands",
             R"({"confirm":true,"reason":"远程恢复测试"})", adminToken, kKeyC);
    tests.check(restart.code == 202 &&
                    envelope(restart).value("data").toObject().value(QStringLiteral("status")) ==
    ...
    bool settled = false;
    const auto adminFlows =
        call(app, crow::HTTPMethod::GET, "/api/v1/admin/flows?userId=1", {}, adminToken);
    ...
    const std::string commandNo = envelope(restart)
                                      ...
                                      .toString()
                                      .toStdString();
    adminStations.completeDueCommands(std::chrono::system_clock::now() + std::chrono::seconds(3));
    const auto completedCommand = call(
        app, crow::HTTPMethod::GET, "/api/v1/admin/device-commands/" + commandNo, {}, adminToken);
    tests.check(completedCommand.code == 200 &&
    ...
    tests.check(
        call(app, crow::HTTPMethod::GET, "/api/v1/admin/stats/revenue?bucket=day", {}, adminToken)
                .code == 200,
        "revenue statistics respond");
    tests.check(call(app, crow::HTTPMethod::GET, "/api/v1/admin/stats/charger-status?stationId=1",
                     {}, adminToken)
                        .code == 200,
                "charger status statistics respond");
    const auto auditLogs = call(app, crow::HTTPMethod::GET,
                                "/api/v1/admin/audit-logs?action=FORCE_RELEASE", {}, adminToken);
    tests.check(auditLogs.code == 200 &&
                    envelope(auditLogs).value("data").toObject().value("items").toArray().size() >=
                        1,
                "audit trail records the force release");
    tests.check(call(app, crow::HTTPMethod::GET, "/api/v1/admin/audit-logs", {}, userToken).code ==
                    401,
                "audit logs reject non-admin tokens");

    const auto backup =
        call(app, crow::HTTPMethod::POST, "/api/v1/admin/backups", "{}", adminToken, kKeyC);
    tests.check(backup.code == 202, "backup orchestration returns 202");
    const std::string backupNo = envelope(backup)
                                     .value("data")
                                     .toObject()
                                     ...
                                     .toStdString();
    const auto verified =
        call(app, crow::HTTPMethod::POST, "/api/v1/admin/backups/" + backupNo + "/verifications",
             "{}", adminToken, kKeyJ);
    ...
    const auto duplicateTask = call(app, crow::HTTPMethod::POST, "/api/v1/admin/ml-tasks",
                                    R"({"taskType":"TRAIN"})", adminToken, kKeyL);
    tests.check(duplicateTask.code == 202 &&
    ...
    agedTask.createdAt = std::chrono::duration_cast<std::chrono::seconds>(
    ...
    const auto agedQuery =
        call(app, crow::HTTPMethod::GET, "/api/v1/admin/ml-tasks/MLAGED0000001", {}, adminToken);
    tests.check(agedQuery.code == 200 && envelope(agedQuery).value("data").toObject().value(
                                             "status") == QStringLiteral("TIMED_OUT"),
                "overdue ml task transitions to TIMED_OUT");
    tests.check(
        call(app, crow::HTTPMethod::GET, "/api/v1/admin/predictions", {}, adminToken).code == 200,
