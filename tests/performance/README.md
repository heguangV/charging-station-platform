...
| `run_load_tests.py` | 编排器：注册 3000 账号 → 充值 → 创建压测站 → 48 充电 + 50 排队 → 100 在线会话 → 容量保持 → 20rps 持续 / 50rps 峰值吞吐 → 清理 → 删除重建数据库；输出 `load-report.json` 与逐项 PASS/FAIL |
| `README.md` | 本说明 |
...
- 3000 个账号全部经真实注册 API（`/auth/sms/code` + `/auth/register`）创建，
  用户名 `lt_0000000`…`lt_0002999`、手机号 `13800000000`…`13800002999`；
  不使用任何预置/演示账号。
...
- 峰值阶段：50 rps 60 秒。允许的响应仅为 200/201/204 与 429+retryAfter（平台的
  受控削峰即为“承受短时峰值”的实现方式，作为通过率构成如实记录，不算失败）；
  断言连接失败、5xx、其他状态码为 0。
- 恢复阶段：峰值后 5 秒（桶回满）再以 20 rps 跑 15 秒，断言 0 429，证明限流器
  恢复无粘滞。
...
- 单 IP 限流把 20/s 稳定 + 50 突发写进了服务端（`// NFR-P-05`），因此“50 rps 短时
  峰值”的量化含义 = burst 吸收 ~50 个请求 + 每桶 20/s 稳定通过，超出部分 429
  节流；本套件按此断言（0 失败/0 5xx），并把 2xx/429 分账记录进证据。
...
- 峰值阶段结束后 429 属预期受控行为，不计入“失败”；持续与恢复阶段出现 429 才是
  平台缺陷。
