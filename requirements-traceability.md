...
| 设备通信模拟器 | UC-X-01 | `tools/device_link_sim` 独立 CMake、固定数组帧编解码和非法帧测试 | 独立 `device_link_core_test` 已通过；S1～S4 的连接、5 桩并发和 GUI 仍待实现 | 部分完成 |
| 安卓用户端 | Android 用户端 | `apps/mobile` Qt Quick 用户端、真实 REST、短信登录、站点、订单、资料和拍照头像 | Qt Android kit 构建、真机 REST 联调、摄像头权限与上传验收 | 部分完成：源码与独立 CMake 已实现，缺 Android SDK/Qt Multimedia 环境验收 |
