...
ApiReply failure(int status, const QString& code, const QString& message)
{
    return {{},
            code,
            message.isEmpty() ? QStringLiteral("网络服务不可用，请稍后重试") : message,
            status};
}
} // namespace
...
struct ApiClient::PendingBinaryRequest
{
    QString path;
    QHash<QByteArray, QByteArray> headers;
...
{
    baseUrl_.setPath(baseUrl_.path().endsWith(QLatin1Char('/'))
                         ? baseUrl_.path()
...
{
    QUrl target = urlFor(path);
    target.setQuery(query);
    enqueue("GET", target.toString(), {}, {}, std::move(handler));
}

void ApiClient::getBinary(const QString& path, const QHash<QByteArray, QByteArray>& headers,
                          BinaryHandler handler)
{
    QHash<QByteArray, QByteArray> requestHeaders = headers;
    requestHeaders.insert("X-Request-ID",
                          QUuid::createUuid().toString(QUuid::WithoutBraces).toUtf8());
    sendBinary(std::make_shared<PendingBinaryRequest>(
        PendingBinaryRequest{path, std::move(requestHeaders), std::move(handler), 0}));
}
...
void ApiClient::getBytes(const QString& path, BytesHandler handler)
{
    ...
    connect(reply, &QNetworkReply::finished, this,
            [reply, handler = std::move(handler)]
            {
                ...
                handler(reply->error() == QNetworkReply::NoError ? body : QByteArray{}, contentType,
                        status);
                reply->deleteLater();
            });
}
...
void ApiClient::send(const std::shared_ptr<PendingRequest>& pending)
{
    ...
    else if (pending->method == "GET")
        reply = manager_->get(request);
    else if (pending->method == "PUT")
        reply = manager_->put(request, pending->body);
    else if (pending->method == "DELETE")
        reply = manager_->sendCustomRequest(request, "DELETE", pending->body);
    else
        reply = manager_->post(request, pending->body);
    ...
    connect(reply, &QNetworkReply::finished, this,
            [this, reply, pending]
            {
                ...
                const ApiReply result =
                    parseEnvelope(status, reply->readAll(), transportError, reply->errorString());
                reply->deleteLater();
...
void ApiClient::sendBinary(const std::shared_ptr<PendingBinaryRequest>& pending)
{
    QNetworkRequest request(urlFor(pending->path));
    request.setTransferTimeout(10000);
    request.setRawHeader("Accept", "image/png,image/jpeg,image/bmp");
    if (!accessToken_.isEmpty())
        request.setRawHeader("Authorization", "Bearer " + accessToken_.toUtf8());
    for (auto it = pending->headers.cbegin(); it != pending->headers.cend(); ++it)
        request.setRawHeader(it.key(), it.value());
    ...
    connect(reply, &QNetworkReply::finished, this,
            [this, reply, pending]
            {
                const int status =
                    reply->attribute(QNetworkRequest::HttpStatusCodeAttribute).toInt();
                const QNetworkReply::NetworkError transportError = reply->error();
                const BinaryApiReply result = parseBinaryReply(
                    status, reply->readAll(),
                    reply->header(QNetworkRequest::ContentTypeHeader).toByteArray(),
                    reply->rawHeader("ETag"), transportError, reply->errorString());
                reply->deleteLater();
                if (result.code == QStringLiteral("NetworkError") && pending->attempts == 0 &&
                    isRetryableTransportError(transportError))
                {
                    ++pending->attempts;
                    QTimer::singleShot(250, this, [this, pending] { sendBinary(pending); });
                    return;
                }
                pending->handler(result);
            });
                                  ...
                                  const QString& transportMessage)
{
    ...
    if (transportError != QNetworkReply::NoError)
    {
        const QString message =
            transportMessage.compare(QStringLiteral("Unknown error"), Qt::CaseInsensitive) == 0
                ? QString()
                : transportMessage;
        return failure(httpStatus, QStringLiteral("NetworkError"), message);
    }
    Q_UNUSED(parseError)
    return failure(httpStatus, QStringLiteral("InvalidResponse"),
                   QStringLiteral("服务返回格式无效"));
}

BinaryApiReply ApiClient::parseBinaryReply(int httpStatus, const QByteArray& payload,
                                           const QByteArray& contentType, const QByteArray& etag,
                                           QNetworkReply::NetworkError transportError,
                                           const QString& transportMessage)
{
    if (httpStatus == 304)
        return {{}, contentType, etag, {}, {}, httpStatus};
    if (transportError == QNetworkReply::NoError && httpStatus >= 200 && httpStatus < 300)
        return {payload, contentType, etag, {}, {}, httpStatus};

    const ApiReply failureReply =
        parseEnvelope(httpStatus, payload, transportError, transportMessage);
    return {{}, contentType, etag, failureReply.code, failureReply.message, httpStatus};
}
...
QUrl ApiClient::urlFor(const QString& path) const
{
    ...
    if (incoming.isValid() && !incoming.scheme().isEmpty())
        return incoming;
