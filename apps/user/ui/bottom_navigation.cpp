...
QIcon iconFor(BottomNavigation::Item item, const QColor& color)
{
    ...
    {
        p.drawPolyline(QPolygonF{{4, 11}, {12, 4}, {20, 11}});
        p.drawRoundedRect(QRectF(6, 10, 12, 10), 2, 2);
        p.drawLine(QPointF(10, 20), QPointF(10, 15));
    }
    else if (item == BottomNavigation::Item::Orders)
    {
        p.drawRoundedRect(QRectF(6, 3, 12, 18), 2, 2);
        p.drawLine(QPointF(9, 9), QPointF(15, 9));
    ...
    else
    {
        ...
        p.drawArc(QRectF(5, 11, 14, 11), 195 * 16, 150 * 16);
    }
    return QIcon(pixmap);
}
} // namespace
BottomNavigation::BottomNavigation(QWidget* parent) : QWidget(parent)
{
    ...
    orders_ = new QToolButton;
    profile_ = new QToolButton;
    home_->setText(QStringLiteral("首页"));
    orders_->setText(QStringLiteral("订单"));
    profile_->setText(QStringLiteral("我的"));
