...
QWidget* UserMainWindow::createProfilePage()
{
    ...
    connect(avatar, &QPushButton::clicked, this,
            [this]
            {
                ...
                userApi_->uploadAvatar(image, QFileInfo(file).fileName(),
                                       [this](ApiReply reply)
                                       {
                                           if (!reply.ok())
                                           {
    ...
    connect(save, &QPushButton::clicked, this,
            [this]
            {
                ...
                if (userApi_)
                {
                    userApi_->updateProfile(nicknameEdit_->text().trimmed(), profileVersion_,
                                            [this](ApiReply reply)
                                            {
                                                if (!reply.ok())
                                                {
    ...
    connect(recharge, &QPushButton::clicked, this,
            [this]
            {
                ...
                if (userApi_)
                {
                    userApi_->recharge(qRound(value * 100),
                                       [this](ApiReply reply)
                                       {
                                           if (!reply.ok())
                                           {
                ...
                if (userApi_)
                {
                    userApi_->logout(
                        [this](ApiReply reply)
                        {
                            ...
                            notify(reply.ok() ? QStringLiteral("已退出登录") : reply.message,
                                   !reply.ok());
                            showLogin();
                        });
                    return;
    ...
    connect(deleteAccount, &QPushButton::clicked, this,
            [this]
            {
                ...
                if (QMessageBox::warning(
                        this, QStringLiteral("确认注销账户"),
                        QStringLiteral(
                            "注销后将撤销全部登录会话并匿名化账户信息。此操作不可撤销，是否继续？"),
                        QMessageBox::Yes | QMessageBox::No, QMessageBox::No) != QMessageBox::Yes)
                    return;
                userApi_->requestPasswordResetCode(
                    phoneEdit_->text(),
                    [this](ApiReply codeReply)
                    {
                        if (!codeReply.ok())
                        {
                        ...
                        const QString smsCode =
                            QInputDialog::getText(this, QStringLiteral("验证注销"),
                                                  QStringLiteral("请输入注销验证码"),
                                                  QLineEdit::Normal, {}, &ok)
                                .trimmed();
                        if (!ok || smsCode.isEmpty())
                            return;
                        userApi_->deleteAccount(smsCode,
                                                [this](ApiReply deleteReply)
                                                {
                                                    if (!deleteReply.ok())
                                                    {
