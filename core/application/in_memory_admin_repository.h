...
{
  public:
    InMemoryAdminRepository(ChargingRepository& charging, UserAccountRepository& accounts,
                            bool withDemoAdmin);

    void withTransaction(const std::function<void()>& work) override;
    std::optional<AdminAccount> findAdminByUsername(std::string_view username) override;
    std::optional<AdminAccount> findAdminById(std::int64_t id) override;
    AdminAccountPage adminAccounts(const AdminAccountQuery& query) override;
    AdminAccountWriteResult createAdminAccount(std::int64_t actorAdminId, std::string_view username,
                                               std::string_view passwordHash,
                                               std::string_view reason, std::int64_t at,
                                               AdminAccount& created) override;
    AdminAccountWriteResult updateAdminAccountStatus(std::int64_t actorAdminId,
                                                     std::int64_t adminId, int status,
                                                     std::string_view reason,
                                                     std::int64_t expectedVersion, std::int64_t at,
                                                     AdminAccount& updated) override;
    AdminAccountWriteResult changeAdminAccountPassword(
        std::int64_t actorAdminId, std::int64_t adminId, std::string_view expectedCurrentHash,
        std::string_view newPasswordHash, std::int64_t at, AdminAccount& updated) override;
    ...
    AccountWriteResult updateManagedUserStatus(std::int64_t actorAdminId, std::int64_t userId,
    ...
    bool createStationWithChargers(Station& station, const InitialChargerSpec& spec) override;
    bool saveStation(const Station& station) override;
    std::vector<Station> stations(std::optional<int> status, std::optional<std::string> adcode,
    ...
    std::optional<PriceAdjustment> effectivePriceAdjustment(std::int64_t stationId, int chargerType,
                                                            std::int64_t at) override;
    void addDeviceCommand(const DeviceCommand& command) override;
    std::optional<DeviceCommand> deviceCommand(const std::string& commandNo) override;
    ...
    std::vector<ChargingOrder> settledOrders(std::int64_t fromAt, std::int64_t toAt,
  ...
  private:
    ...
    AdminAccount* findAdminByIdLocked(std::int64_t id);
