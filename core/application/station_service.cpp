...
std::vector<ChargerType> collectSupportedTypes(const std::vector<Charger>& chargers)
{
...
StationSummary summarize(const Station& station, const std::vector<Charger>& chargers,
                         const RegionTariff& tariff, std::int64_t distanceMeter)
{
    ...
    summary.totalPriceCentPerKwh = tariff.electricityCentPerKwh + tariff.serviceCentPerKwh;
    summary.distanceMeter = distanceMeter;
    for (const Charger& charger : chargers)
    {
        if (charger.status == ChargerStatus::Idle)
            ++summary.idleCount;
        if (charger.status == ChargerStatus::Idle || charger.status == ChargerStatus::Occupied)
        {
...
std::int64_t StationService::haversineMeter(const std::int64_t leftLatitudeE6,
                                            const std::int64_t leftLongitudeE6,
                                            const std::int64_t rightLatitudeE6,
                                            const std::int64_t rightLongitudeE6)
{
    constexpr double earthRadiusMeter = 6371000.0;
    const double latitude1 = leftLatitudeE6 / 1e6 * pi / 180.0;
    const double latitude2 = rightLatitudeE6 / 1e6 * pi / 180.0;
    const double deltaLatitude = (rightLatitudeE6 - leftLatitudeE6) / 1e6 * pi / 180.0;
    const double deltaLongitude = (rightLongitudeE6 - leftLongitudeE6) / 1e6 * pi / 180.0;
    const double a = std::sin(deltaLatitude / 2) * std::sin(deltaLatitude / 2) +
                     std::cos(latitude1) * std::cos(latitude2) * std::sin(deltaLongitude / 2) *
                         std::sin(deltaLongitude / 2);
    return static_cast<std::int64_t>(
        2.0 * earthRadiusMeter * std::atan2(std::sqrt(a), std::sqrt(1.0 - a)) + 0.5);
}

StationListResult StationService::nearbyStations(const std::optional<std::int64_t> latitudeE6,
                                                 const std::optional<std::int64_t> longitudeE6,
                                                 const std::string& keyword,
                                                 const std::optional<ChargerType> chargerType,
                                                 ...
                                                 const std::chrono::system_clock::time_point now)
{
    ...
    for (const Station& station : repository_.stations())
    {
        if (!station.enabled)
            continue;
        const auto stationChargers = repository_.chargers(station.id, chargerType, std::nullopt);
        ...
        summaries.push_back(summarize(
            station, repository_.chargers(station.id, std::nullopt, std::nullopt), *tariff,
    ...
    for (std::size_t index = first;
         index < summaries.size() && result.items.size() < static_cast<std::size_t>(pageSize);
         ++index)
    {
                              ...
                              const std::chrono::system_clock::time_point now)
{
    ...
    detail.summary = summarize(
        *station, repository_.chargers(stationId, std::nullopt, std::nullopt), *tariff, 0);
    detail.businessHours = station->businessHours;
    detail.supportedTypes =
        collectSupportedTypes(repository_.chargers(stationId, std::nullopt, std::nullopt));
    return {core::domain::ErrorCode::Ok, detail};
}

ServiceResult<ChargerPage> StationService::stationChargers(const std::int64_t stationId,
...
StationService::stationQuote(const std::int64_t stationId, const ChargerType type,
                             const std::chrono::system_clock::time_point now)
{
    const auto station = repository_.station(stationId);
    if (!station)
    ...
    const std::int64_t adjustmentBp =
        adjustmentLookup_ ? adjustmentLookup_(stationId, static_cast<int>(type), nowSeconds) : 0;
    const PriceBreakdown breakdown =
        computePrice(*tariff, static_cast<int>(waiting), static_cast<int>(adjustmentBp));
    StationQuote quote;
