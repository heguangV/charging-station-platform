...
bool isActiveStatus(int status) { return isActiveFlowStatus(status); }

bool matchesAudit(const AuditEvent &event, const AuditEventQuery &query) {
  if (!query.actorId.empty() &&
      std::to_string(event.actorAdminId) != query.actorId)
    return false;
...
InMemoryAdminRepository::findAdminByUsername(const std::string_view username) {
  ...
  return found == admins_.end() ? std::nullopt
                                : std::optional<AdminAccount>(found->second);
}

std::optional<AdminAccount> InMemoryAdminRepository::findAdminById(
...
AdminUserPage InMemoryAdminRepository::listManagedUsers(
  ...
  for (const auto &account : all) {
    ...
    if (query.phoneExact && account.phone != *query.phoneExact)
      continue;
    if (query.phoneLast4 &&
        (account.phone.size() != 11 ||
         account.phone.substr(7) != *query.phoneLast4))
      continue;
  ...
  const std::size_t first =
      static_cast<std::size_t>(query.page - 1) * query.pageSize;
  for (std::size_t index = first;
       index < filtered.size() &&
       page.items.size() < static_cast<std::size_t>(query.pageSize);
       ++index) {
...
std::optional<UserAccount> InMemoryAdminRepository::findManagedUser(
    const std::int64_t id) {
...
AccountWriteResult InMemoryAdminRepository::updateManagedUserStatus(
    ...
    const int status, const std::string_view reason,
    const std::int64_t expectedVersion, const std::int64_t at,
    UserAccount &updated) {
  std::lock_guard lock(mutex_);
  ...
  if (write == AccountWriteResult::Success) {
    audit_.push_back(AuditEvent{
        actorAdminId, status == 0 ? "USER_FROZEN" : "USER_UNFROZEN", "USER",
        std::to_string(userId), std::string(reason), at});
  }
  return write;
}
...
std::vector<AuditEvent> InMemoryAdminRepository::auditEvents(
...
bool InMemoryAdminRepository::createStationWithChargers(
  ...
  for (int index = 1; index <= spec.count; ++index) {
    ...
    charger.code = station.code + (spec.chargerType == ChargerType::DcFast
                                       ? "-DC-" : "-AC-") + suffix;
...
std::vector<Station> InMemoryAdminRepository::stations(
  ...
  for (Station &station : charging_.stations()) {
    ...
    if (!keyword.empty() && station.name.find(keyword) == std::string::npos &&
        station.address.find(keyword) == std::string::npos &&
        station.code != keyword)
      continue;
...
std::vector<RegionTariff> InMemoryAdminRepository::tariffVersions(
    std::optional<std::string> adcode) {
  return charging_.tariffVersions(std::move(adcode));
}

bool InMemoryAdminRepository::tariffOverlaps(const std::string &adcode,
...
std::int64_t InMemoryAdminRepository::addPriceAdjustment(
    const PriceAdjustment &adjustment) {
  std::lock_guard lock(mutex_);
...
std::optional<PriceAdjustment> InMemoryAdminRepository::effectivePriceAdjustment(
  ...
  for (const auto &adjustment : adjustments_) {
    if (adjustment.stationId != stationId ||
        adjustment.chargerType != chargerType)
      continue;
...
std::optional<DeviceCommand> InMemoryAdminRepository::deviceCommand(
    const std::string &commandNo) {
  std::lock_guard lock(mutex_);
  const auto found = deviceCommands_.find(commandNo);
  return found == deviceCommands_.end()
             ? std::nullopt
...
std::vector<DeviceCommand> InMemoryAdminRepository::dueDeviceCommands(
...
std::optional<ChargingFlow> InMemoryAdminRepository::activeFlowOnCharger(
    const std::int64_t chargerId) {
  for (const auto &flow : charging_.allFlows()) {
    if (flow.chargerId && *flow.chargerId == chargerId &&
        isActiveStatus(flow.status))
      return flow;
  }
  return std::nullopt;
}

bool InMemoryAdminRepository::stationHasActiveFlow(
...
std::vector<ChargingOrder> InMemoryAdminRepository::settledOrders(
  ...
  for (const auto &order : charging_.allOrders()) {
    if (order.status != static_cast<int>(FlowStatus::Completed) ||
        !order.settledAt)
      continue;
...
std::optional<MlTask> InMemoryAdminRepository::runningMlTask(
...
std::vector<MlTask> InMemoryAdminRepository::overdueMlTasks(
  ...
  for (const auto &[taskNo, task] : mlTasks_) {
    ...
    const std::int64_t deadline =
        task.taskType == "TRAIN" ? trainDeadline : predictDeadline;
...
std::optional<MlTask> InMemoryAdminRepository::mlTask(
    const std::string &taskNo) {
  std::lock_guard lock(mutex_);
  const auto found = mlTasks_.find(taskNo);
  return found == mlTasks_.end() ? std::nullopt
                                 : std::optional<MlTask>(found->second);
}
...
bool InMemoryAdminRepository::tryFinishMlTask(const MlTask &task,
                                              const bool allowTimedOut) {
  std::lock_guard lock(mutex_);
  const auto found = mlTasks_.find(task.taskNo);
  if (found == mlTasks_.end())
    return false;
  const bool live = found->second.status == "PENDING" ||
...
std::optional<BackupRecord> InMemoryAdminRepository::backup(
    const std::string &backupNo) {
  std::lock_guard lock(mutex_);
  const auto found = backups_.find(backupNo);
  return found == backups_.end() ? std::nullopt
                                 : std::optional<BackupRecord>(found->second);
}
...
bool InMemoryAdminRepository::verifyBackupSnapshot(
...
void InMemoryAdminRepository::cleanupAdminRecords(const std::int64_t now) {
  ...
  audit_.erase(std::remove_if(audit_.begin(), audit_.end(),
               ...
               audit_.end());
  for (auto iterator = deviceCommands_.begin();
       iterator != deviceCommands_.end();) {
    if (iterator->second.completedAt &&
        *iterator->second.completedAt < now - retention) {
