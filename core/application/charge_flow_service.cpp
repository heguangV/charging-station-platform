...
std::optional<int> queuePositionOf(ChargingRepository& repository, const ChargingFlow& flow)
{
...
FlowView toFlowView(const ChargingFlow& flow, std::optional<int> queuePosition)
{
    FlowView view;
...
bool validFlowVersion(const ChargingFlow& flow, const std::int64_t flowVersion)
{
...
// 创建充电请求（事务内）：为公平起见先循环晋级该站点/桩型的排队流程，再分配空闲桩——
// 分到则生成 5 分钟报价（状态 20）并记录设备占用事件，否则入 FIFO 队列（状态 10）。
ServiceResult<FlowView>
ChargeFlowService::createFlow(const std::int64_t userId, const std::int64_t stationId,
                              const int chargerType,
                              const std::optional<std::int64_t> preferredChargerId,
    ...
    repository_.withTransaction(
        [&]
        {
            ...
            if (preferredChargerId)
            {
                ...
                if (candidate && candidate->stationId == stationId && candidate->type == type &&
                    candidate->status == ChargerStatus::Idle)
                {
                    allocated = candidate;
                }
                else
            ...
            {
                const auto idle = repository_.chargers(stationId, type, ChargerStatus::Idle);
                if (!idle.empty())
            ...
            if (allocated)
            {
                ...
                repository_.addChargerStatusEvent(ChargerStatusEvent{
                    allocated->id, allocated->stationId, static_cast<int>(ChargerStatus::Idle),
                    static_cast<int>(ChargerStatus::Occupied), "ALLOCATED", nowSeconds});
                ...
                const std::int64_t adjustmentBp =
                    adjustmentLookup_ ? adjustmentLookup_(stationId, chargerType, nowSeconds) : 0;
                const PriceBreakdown breakdown = computePrice(*tariff, static_cast<int>(waiting),
                                                              static_cast<int>(adjustmentBp));
                FlowQuote quote;
            ...
            if (queued)
            {
                ...
                queuePosition = static_cast<int>(repository_.queue(stationId, type).size());
            }
            repository_.addFlowEvent(FlowEvent{flow.flowNo, 0, flow.status, "CREATED", nowSeconds});
...
ActiveFlowView ChargeFlowService::activeFlow(const std::int64_t userId,
                                             const std::chrono::system_clock::time_point now)
{
    (void)now;
    ...
    view.flow = toFlowView(*flow, flow->status == static_cast<int>(FlowStatus::Queued)
                                      ? queuePositionOf(repository_, *flow)
                                      : std::nullopt);
    return view;
}

ServiceResult<FlowView> ChargeFlowService::flowView(const std::int64_t userId,
...
// 确认报价（事务内，状态 20→30）：创建订单并写入价格快照，流程转预约（版本
// +1、清除报价），设备保持占用。
ServiceResult<QuoteConfirmationView>
ChargeFlowService::confirmQuote(const std::int64_t userId, const std::string& flowNo,
                                ...
                                const std::chrono::system_clock::time_point now)
{
    ...
    repository_.withTransaction(
        [&]
        {
            ...
            if (flow->status != static_cast<int>(FlowStatus::PendingQuote) || !flow->quote ||
                flow->quote->quoteNo != quoteNo)
            {
            ...
            const auto tariff =
                station ? repository_.effectiveTariff(station->adcode, nowSeconds) : std::nullopt;
            if (!station || !tariff)
            {
                error = core::domain::ErrorCode::ChargerUnavailable;
                return;
            ...
            repository_.addFlowEvent(
                FlowEvent{flowNo, flow->status, updated.status, "QUOTE_CONFIRMED", nowSeconds});
...
// 取消流程（事务内）：已占用设备则回 Idle 并记录状态事件，随后触发队列晋级；未占用设备仅出队；
// 关联订单若仍在进行中一并置 70，最后清除钱包镜像的进行中标记。
ServiceResult<FlowView> ChargeFlowService::cancel(const std::int64_t userId,
                                                  const std::string& flowNo,
                                                  ...
                                                  const std::chrono::system_clock::time_point now)
{
    ...
    repository_.withTransaction(
        [&]
        {
            ...
            const bool cancellable = flow->status == static_cast<int>(FlowStatus::Queued) ||
                                     flow->status == static_cast<int>(FlowStatus::PendingQuote) ||
            ...
            if (flow->chargerId)
            {
                if (auto charger = repository_.charger(*flow->chargerId))
                {
                    ...
                    repository_.addChargerStatusEvent(ChargerStatusEvent{
                        charger->id, charger->stationId, fromStatus,
                ...
                repository_.saveFlow(updated);
                repository_.addFlowEvent(
                    FlowEvent{flowNo, flow->status, updated.status, reasonCode, nowSeconds});
...
// 开始充电（事务内）：预约未过期且设备仍被占用才放行；余额下限取全市最低启动额与客户端下限的较大者（BR-04）。
                         ...
                         const std::chrono::system_clock::time_point now)
{
    ...
    repository_.withTransaction(
        [&]
        {
            ...
            const std::int64_t floor = std::max(minimumStartBalanceCent,
                                                balanceFloorCent.value_or(minimumStartBalanceCent));
            if (wallet.debtCent > 0)
            {
                error = core::domain::ErrorCode::DebtOutstanding;
                return;
...
// 查询充电进度（只读）：按订单快照的时间倍率把真实时间换算为模拟时长，推算电量（毫瓦时）、金额（分）与
// SOC，不产生任何写入。
ServiceResult<ChargeProgressView>
ChargeFlowService::progress(const std::int64_t userId, const std::string& flowNo,
                            const std::chrono::system_clock::time_point now)
{
    const auto flow = repository_.flow(flowNo);
    if (!flow || flow->userId != userId)
        return {core::domain::ErrorCode::NotFound, std::nullopt};
    if (flow->status != static_cast<int>(FlowStatus::Charging) || !flow->startedAt)
    {
    ...
    if (!order)
        return {core::domain::ErrorCode::NotFound, std::nullopt};
    const auto nowSeconds = unixSeconds(now);
    const std::int64_t simulatedSeconds =
        std::max<std::int64_t>(0, (nowSeconds - *flow->startedAt) * order->timeScale);
    const std::int64_t energyMwh = energyMwhForDuration(order->powerWatt, simulatedSeconds);
    const int totalPrice = order->electricityPriceCentPerKwh + order->servicePriceCentPerKwh;
    ...
    view.simulatedSoc = std::min(
        100, initialSimulatedSoc + static_cast<int>(energyMwh * 80 / simulatedBatteryCapacityMwh));
...
ChargeFlowService::settle(const std::int64_t userId, const std::string& flowNo,
                          const std::int64_t flowVersion, const std::string& reasonCode,
                          const std::chrono::system_clock::time_point now)
{
    if (!validReasonCode(reasonCode))
        return {core::domain::ErrorCode::ValidationFailed, std::nullopt};
    ...
    try
    {
        repository_.withTransaction(
            [&]
            {
                ...
                const std::int64_t simulatedSeconds =
                    std::max<std::int64_t>(0, (nowSeconds - *order->startedAt) * order->timeScale);
                ...
                [&]
                {
                    ...
                    if (!current || current->userId != userId ||
                        (current->status != static_cast<int>(FlowStatus::Charging) &&
                         ...
                         current->status != static_cast<int>(FlowStatus::SettlementFailed)))
                    {
                        return;
                    }
                    ...
                    repository_.addFlowEvent(FlowEvent{flowNo, current->status, failed.status,
                                                       "SETTLEMENT_FAILED", nowSeconds});
                    walletMirror_.setActiveFlowFlag(userId, true);
                });
        }
        catch (...)
...
ServiceResult<OrderPage> ChargeFlowService::orders(const std::int64_t userId,
                                                   ...
                                                   const std::int64_t toAt, const std::string& sort,
                                                   const int page, const int pageSize) const
{
    ...
    for (std::size_t index = first;
         index < all.size() && result.items.size() < static_cast<std::size_t>(pageSize); ++index)
    {
...
ServiceResult<SettlementReceipt> ChargeFlowService::receipt(const std::int64_t userId,
...
// 运行时巡检（事务内）：统一把过期报价/预约流程置
// 90，释放设备、记录状态事件、作废订单并触发队列晋级。
void ChargeFlowService::runMaintenance(const std::chrono::system_clock::time_point now)
{
    const auto nowSeconds = unixSeconds(now);
    repository_.withTransaction(
        [&]
        {
            ...
            for (const auto& flow :
                 repository_.flowsWithStatus(static_cast<int>(FlowStatus::PendingQuote)))
            {
...
// 启动恢复（事务内）：按状态 10~50/80 扫描遗留流程并恢复钱包镜像标记，返回恢复条数。
int ChargeFlowService::recoverAtStartup(const std::chrono::system_clock::time_point now)
{
    int recovered = 0;
    repository_.withTransaction(
        [&]
...
// 队列晋级单步（须在事务内调用）：先清理队首失效条目（非排队状态直接出队），再为队首流程分配空闲桩并生成新报价（状态
// 10→20）； 无空闲桩、无价目表或队列为空时不做任何变更。
ServiceResult<FlowView>
ChargeFlowService::promoteQueueLocked(const std::int64_t stationId, const ChargerType type,
                                      const std::chrono::system_clock::time_point now)
{
    const auto nowSeconds = unixSeconds(now);
    ...
    while (!queue.empty() && !allocated)
    {
        ...
        if (!candidateFlow || candidateFlow->status != static_cast<int>(FlowStatus::Queued))
        {
            queue.pop_front();
    ...
    repository_.addChargerStatusEvent(ChargerStatusEvent{
        allocated->id, allocated->stationId, static_cast<int>(ChargerStatus::Idle),
        static_cast<int>(ChargerStatus::Occupied), "PROMOTED", nowSeconds});
    ...
    const PriceBreakdown breakdown = computePrice(
        *tariff, static_cast<int>(repository_.queue(stationId, type).size()),
        adjustmentLookup_
            ? static_cast<int>(adjustmentLookup_(stationId, static_cast<int>(type), nowSeconds))
            : 0);
    ...
    repository_.addFlowEvent(
        FlowEvent{promotedFlowNo, flow->status, promoted.status, "PROMOTED", nowSeconds});
    return {core::domain::ErrorCode::Ok, toFlowView(promoted, std::nullopt)};
}

// 管理端强制释放（事务内）：设备置为调用方指定状态并版本 +1，流程与进行中订单一并置
// 70，随后触发队列晋级。
ServiceResult<FlowView>
ChargeFlowService::adminForceRelease(const std::string& flowNo, const std::string& reason,
                                     const int nextChargerStatus, const std::int64_t flowVersion,
                                     const std::chrono::system_clock::time_point now)
{
    if (!validReasonText(reason) || (nextChargerStatus != 0 && nextChargerStatus != 2 &&
    ...
    repository_.withTransaction(
        [&]
        {
            ...
            const bool releasable = flow->status == static_cast<int>(FlowStatus::PendingQuote) ||
                                    flow->status == static_cast<int>(FlowStatus::Reserved);
            if (!releasable)
            {
                error = core::domain::ErrorCode::InvalidStateTransition;
            ...
            repository_.saveFlow(updated);
            repository_.addFlowEvent(
                FlowEvent{flowNo, flow->status, updated.status, reason, nowSeconds});
            if (const auto order = repository_.orderByFlow(flowNo))
            {
                if (isActiveFlowStatus(order->status))
                {
...
ChargeFlowService::adminControlledSettle(const std::string& flowNo, const std::string& reason,
                                         ...
                                         const std::chrono::system_clock::time_point now)
{
    if (!validReasonText(reason) || (nextChargerStatus != 0 && nextChargerStatus != 4))
        return {core::domain::ErrorCode::ValidationFailed, std::nullopt};
    const auto nowSeconds = unixSeconds(now);
    core::domain::ErrorCode error = core::domain::ErrorCode::Ok;
    std::optional<SettlementReceipt> receipt;
    repository_.withTransaction(
        [&]
        {
            ...
            const bool settleable = flow->status == static_cast<int>(FlowStatus::Charging) ||
                                    flow->status == static_cast<int>(FlowStatus::SettlementFailed);
            if (!settleable)
            {
                error = core::domain::ErrorCode::InvalidStateTransition;
...
void ChargeFlowService::promoteAvailable(const std::int64_t stationId, const ChargerType type,
                                         const std::chrono::system_clock::time_point now)
{
    repository_.withTransaction(
        [&]
